 ...
 * Image Storage
 * 
 * Saves all vision-related images to structured directories.
 * 
 * Saves to:
 *   data/uploads/vision/YYYY-MM-DD/
 * 
 * Preserves original image (never modifies/delete).
 */
...
const BASE_DIR = path.resolve('./data/uploads/vision');

/**
 * Save a vision-related image (incident or photo audit).
 * 
 * @param {Buffer|object} mediaData  - whatsapp-web.js media object or raw Buffer
 * @param {object} metadata          - { chatId, sender, senderName, timestamp, messageId, type }
 * @returns {string} savedPath       - absolute path to saved image
 */
function saveVisionImage(mediaData, metadata = {}) {
  ...
  if (!fs.existsSync(dir)) {
    ...
    log.info('Created vision image directory', { dir });
  }

  const ext = getMediaExtension(mediaData) || '.jpg';
  const messageId = (metadata?.messageId || 'unknown').replace(/[^a-zA-Z0-9]/g, '_');
  const type = metadata?.type || 'incident';
  const filename = `${type}-${Date.now()}-${messageId}${ext}`;
  ...
  log.info('Vision image saved', { filePath, size: buffer.length, type: metadata?.type });
  return filePath;
}

/**
 * Save an image from an existing file path (for food-safety pipeline reuse).
 */
function copyImageToVisionDir(sourcePath, metadata = {}) {
  ...
  const filename = `vision-${Date.now()}-${path.basename(sourcePath).replace(/[^a-zA-Z0-9]/g, '_')}`;
  ...
  log.info('Image copied to vision dir', { sourcePath, destPath });
  return destPath;
}

/**
 * Get list of saved images for a date.
 */
...
/**
 * Get total storage used by vision images.
 */
