 ...
 */
async function confirmSubmission({ chatId, sender, senderName, storeId, store, imagePath, ocrResult, ocrConfidence, items, warnings, safetyStatus = '', safetyIssues = [], submissionId: existingSubmissionId }) {
  await ensureTables();
  const now = new Date().toISOString();
  ...
  if (existingSubmissionId) {
    await run(
      ...
      [
        ...
        imagePath || '',
        JSON.stringify(ocrResult || {}),
        JSON.stringify({ items, warnings, safetyStatus, safetyIssues }),
        ocrConfidence || 0,
        now,
