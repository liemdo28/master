...
async function main() {
  ...
  // Run DB migrations before opening the long-lived app connection. On Windows,
  // running sqlite schema bootstrap and migrations concurrently can hold a write lock.
  bootStep('BOOT_STEP_2_DB');
  try {
    const { runMigrations } = require('./migrations/migration-runner');
    await runMigrations();
    const { ensureConfiguredClients } = require('./security/project-client-registry');
    await ensureConfiguredClients();
  } catch (err) {
  ...
  if (typeof apiServer.setWhatsappSession === 'function') {
    apiServer.setWhatsappSession(sessionManager);
  }
