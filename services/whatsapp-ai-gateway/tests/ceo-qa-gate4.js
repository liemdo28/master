/**
 * CEO QA Gate — Minimal diagnostic version
 * Run: node tests/ceo-qa-gate4.js
 */
...
async function run() {
  ...
  console.log('  ... calling getSubmissionStats() ...');
  const stats = await fps.getSubmissionStats();
  console.log('  ... getSubmissionStats done ...');
  ...
  console.log('  ... calling getRecentSubmissions() ...');
  const recent = await fps.getRecentSubmissions(10);
  console.log('  ... getRecentSubmissions done ...');
  ...
  console.log('  ... calling syncSubmission() ...');
  const syncResult = await fpsi.syncSubmission('NONEXISTENT');
  console.log('  ... syncSubmission done ...');
  ...
  console.log('  ... querying DB for confirmed submission ...');
