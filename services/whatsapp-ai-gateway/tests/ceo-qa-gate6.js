/**
 * CEO QA Gate — File-based DB (no hanging)
 * Run: node tests/ceo-qa-gate6.js
 */

process.env.GATEWAY_DB_PATH = './data/test-ceo-qa-final.db';
...
async function run() {
  ...
  const stats = await new Promise((res, rej) => {
    db.get(`SELECT COUNT(*) as total, SUM(CASE WHEN status='CONFIRMED' THEN 1 ELSE 0 END) as confirmed FROM food_safety_submissions`, [], (e, r) => e ? rej(e) : res(r));
  });
  ...
  const recent = await new Promise((res, rej) => {
    db.all(`SELECT * FROM food_safety_submissions ORDER BY created_at DESC LIMIT 10`, [], (e, r) => e ? rej(e) : res(r));
  });
  ...
  const confirmed = await new Promise((res, rej) => {
    db.all(`SELECT * FROM food_safety_submissions WHERE status='CONFIRMED' ORDER BY created_at DESC LIMIT 1`, [], (e, r) => e ? rej(e) : res(r[0] || null));
  });
  ...
  const items = await new Promise((res, rej) => {
    db.all(`SELECT * FROM food_safety_submission_items WHERE submission_id=?`, [confirmed.submission_id], (e, r) => e ? rej(e) : res(r));
  });
  ...
  const confirmed2 = await new Promise((res, rej) => {
    db.all(`SELECT * FROM food_safety_submissions WHERE status='CONFIRMED' AND store='Stone Oak' ORDER BY created_at DESC LIMIT 1`, [], (e, r) => e ? rej(e) : res(r[0] || null));
  });
  ...
  const confirmed3 = await new Promise((res, rej) => {
    db.all(`SELECT * FROM food_safety_submissions WHERE status='CONFIRMED' AND store='Bandera' ORDER BY created_at DESC LIMIT 1`, [], (e, r) => e ? rej(e) : res(r[0] || null));
  });
