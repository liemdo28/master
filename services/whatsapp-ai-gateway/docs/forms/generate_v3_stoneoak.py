"""
generate_v3_stoneoak.py
Generates FoodSafety-StoneOak-v3 — pilot-ready print form.

v3 improvements over v2:
  - Larger write boxes (AM / PM cells: taller for legibility)
  - QR code anchor box bottom-right for camera alignment
  - "STONE OAK PILOT" watermark in header
...
FORM_ID    = "FoodSafety-StoneOak-v3"
VERSION    = "v3.0 — PILOT"
STORE_NAME = "Stone Oak"
PREFIX     = "SO"
OUT_FILE   = "FoodSafety-StoneOak-LineCheck-v3.pdf"
...
ITEMS = [
    ...
    ("Walk-In Cooler",      "≤ 40°F",       True),
    ("Walk-In Freezer",     "≤ 0°F",        True),
    ("Prep Area Cooler",    "≤ 40°F",       False),
    ("Ramen Refrig Bottom", "≤ 40°F",       False),
    ("Ramen Refrig Top",    "≤ 40°F",       False),
    ("Line Freezer",        "≤ 0°F",        True),
    ("Tapas Refrig Bottom", "≤ 40°F",       False),
    ("Tapas Refrig Top",    "≤ 40°F",       False),
    ("Pork Chashu",         "≤ 40°F",       True),
    ("Chicken Chashu",      "≤ 40°F",       True),
    ("Fryer 1",             "≥ 325°F",      True),
    ("Fryer 2",             "≥ 325°F",      True),
    ("Pasta Boiler 1",      "≥ 200°F",      False),
    ("Pasta Boiler 2",      "≥ 200°F",      False),
    ("Seasoned Eggs",       "≥ 100°F",      False),
    ("Bowl Warmers",        "≥ 100°F",      True),
    ("Pork Broth",          "≥ 200°F",      True),
    ("Chicken Broth",       "≥ 200°F",      True),
    ("Veggie Broth",        "≥ 200°F",      False),
]
...
def draw_header(c):
    ...
    c.setFillColor(colors.HexColor("#FF3300"))
    c.roundRect(badge_x, y - 0.32 * inch, 1.05 * inch, 0.26 * inch, 3, fill=1, stroke=0)
    c.setFillColor(colors.white)
    c.setFont("Helvetica-Bold", 8)
    c.drawCentredString(badge_x + 0.525 * inch, y - 0.175 * inch, "STONE OAK PILOT")
...
def draw_footer(c, table_bottom_y):
    ...
    c.setFillColor(colors.HexColor("#555555"))
    line1 = (f"{FORM_ID}  |  Bakudan Ramen Group  |  Stone Oak Pilot  |  "
...
def build():
    ...
    c.setSubject("Food Safety Line Check — Stone Oak Pilot v3")
