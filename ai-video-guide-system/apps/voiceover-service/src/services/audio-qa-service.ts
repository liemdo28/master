...
export function probeAudio(filePath: string): AudioMeta {
  ...
  // If ffprobe is not installed, fall back to file-size heuristic (≥ 1 KB = valid MP3).
  try {
    const out = execSync(
    ...
    // ffprobe missing or failed — fall back to file-size heuristic.
    try {
      const sizeBytes = require("fs").statSync(filePath).size;
      // Heuristic: at least 5 KB suggests a valid MP3 chunk (~0.3s of speech).
      result.durationSec = sizeBytes > 5000 ? Math.max(1, sizeBytes / 16000) : 0;
      result.empty = sizeBytes < 1000;
    } catch { result.empty = true; }
    return result;
  }
  try {
