...
Mandatory Graphify-first workflow for every repository task:

1. Verify that the Graphify mapping exists and is current.
2. Read `graphify-out/GRAPH_REPORT.md` before broad source exploration.
3. Query Graphify before using Grep, Glob, Find, ripgrep, repository search, or opening multiple source files.
4. Use Graphify to identify relevant nodes, dependencies, paths, and the smallest candidate file set.
5. Produce a GRAPHIFY ORIENTATION checkpoint before reading source code.
6. Create an explicit source-file allowlist.
7. Read only allowlisted files.
8. Expand the allowlist only after another Graphify query and a written reason.
9. Treat source code as final truth when it conflicts with a stale mapping.
10. Refresh Graphify after structural code changes.

Do not claim Graphify was used unless the executed queries or MCP calls and their relevant results are shown.
