# Mi-Core Central Command — AGENTS.md
...
Mandatory Graphify-first repository analysis:

Before answering repository questions or modifying code, the agent MUST:

1. Confirm that the Graphify mapping exists.
2. Read `graphify-out/GRAPH_REPORT.md`.
3. Execute at least one task-specific Graphify query.
4. For dependency or impact questions, use Graphify path, traversal, neighbor, or explanation capabilities.
5. Produce a GRAPHIFY ORIENTATION checkpoint.
6. Create a source-file allowlist.
7. Do not run repository-wide search before the checkpoint.
8. Read only allowlisted files.
9. Expand scope only after another Graphify query and explicit justification.
10. Verify graph-derived findings against current source code.
11. Refresh Graphify after structural changes.

No source file may be edited until GRAPHIFY ORIENTATION is complete.

The agent must not claim Graphify usage without listing the query or MCP tools executed and summarizing their relevant outputs.
