...
function parseTime(value, unit) {
    ...
    if (u.includes('giây') || u.includes('second'))
        return n * 1000;
    if (u.includes('phút') || u.includes('minute') || u.includes('min'))
        return n * 60_000;
    if (u.includes('tiếng') || u.includes('giờ') || u.includes('hour'))
        return n * 3_600_000;
    if (u.includes('ngày') || u.includes('day'))
        return n * 86_400_000;
...
function parseReminderCommand(text) {
    ...
    const msgMatch = t.match(/nhắc(?:\s+anh)?\s+(.+?)(?:\s+(?:sau|mỗi|lúc|every|after|at)\s|$)/i) || t.match(/remind(?:\s+me)?\s+(?:to\s+)?(.+?)(?:\s+(?:every|after|at|in)\s|$)/i);
    const message = msgMatch?.[1]?.trim() || extractDefaultMessage(t);
    // INTERVAL: "mỗi X tiếng/phút", "every X hours/minutes"
    const intervalMatch = t.match(/(?:mỗi|every)\s+([\d.,]+)\s*(tiếng|giờ|phút|giây|giây|hour|minute|min|second)/i);
    if (intervalMatch) {
        return {
    ...
    const dailyMatch = t.match(/(?:lúc|at)\s+(\d{1,2})(?::(\d{2}))?\s*(?:giờ\s*)?(sáng|chiều|tối|am|pm)?/i);
    ...
    const onceMatch = t.match(/(?:sau|in|after)\s+([\d.,]+)\s*(tiếng|giờ|phút|giây|hour|minute|min|second)/i);
    if (onceMatch) {
        return {
            message, type: 'once',
