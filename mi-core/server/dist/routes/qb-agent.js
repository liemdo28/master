...
exports.qbAgentRouter = (0, express_1.Router)();
exports.qbAgentRouter.use(requireAgentAuth);
// ── Ping ─────────────────────────────────────────────────────────────────────
exports.qbAgentRouter.get('/ping', (_req, res) => {
    res.json({ ok: true, server: 'mi-core', timestamp: now() });
});
// ── Register ──────────────────────────────────────────────────────────────────
exports.qbAgentRouter.post('/register', (req, res) => {
    const b = req.body || {};
    const machineId = b.machine_id || req.headers['x-machine-id'];
    if (!machineId) {
    ...
    if (existing) {
      ...
      app_version=?, os_version=?, hostname=?, status='online', last_seen_at=? WHERE machine_id=?`)
            .run(b.machine_name, b.store_code, b.store_name, b.location, b.app_version, b.os_version, b.hostname, now(), machineId);
    }
    else {
      ...
      VALUES (?,?,?,?,?,?,?,?,'online',?,?)`)
            .run(machineId, b.machine_name, b.store_code, b.store_name, b.location, b.app_version, b.os_version, b.hostname, now(), now());
    }
    sheetSync.onRegister({ ...b, machine_id: machineId });
    res.json({ ok: true, machine_id: machineId, registered_at: now() });
});
// ── Heartbeat ─────────────────────────────────────────────────────────────────
exports.qbAgentRouter.post('/heartbeat', (req, res) => {
    const b = req.body || {};
    const machineId = b.machine_id || req.headers['x-machine-id'];
    if (!machineId) {
        res.status(400).json({ error: 'machine_id required' });
    ...
    VALUES (?,?,?,?,?,?,?,?,?)`)
        .run(machineId, b.store_code, b.status || 'ok', b.qb_open ? 1 : 0, b.qb_company, b.app_version, b.uptime_seconds, ts, JSON.stringify(b.meta || {}));
...
// ── Event ─────────────────────────────────────────────────────────────────────
exports.qbAgentRouter.post('/event', (req, res) => {
    const b = req.body || {};
    const machineId = b.machine_id || req.headers['x-machine-id'];
    ...
    VALUES (?,?,?,?,?,?,?,?,?)`)
        .run(machineId, b.store_code, b.event_type, b.event_key, b.message, b.severity || 'info', b.occurred_at, now(), JSON.stringify(b.payload || {}));
    res.json({ ok: true });
});
// ── Activity Log Result ───────────────────────────────────────────────────────
exports.qbAgentRouter.post('/activity-log-result', (req, res) => {
    const b = req.body || {};
    const machineId = b.machine_id || req.headers['x-machine-id'];
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
        .run(machineId, b.store_code, b.file_id, b.business_date, b.total_transactions || 0, b.total_amount || 0, b.latest_sales_receipt_date, b.latest_sales_receipt_ref, b.latest_sales_receipt_amount, b.latest_bank_transaction_date, b.latest_reconcile_date, b.local_json_path, b.local_markdown_path, b.metrics_json || '{}', b.warnings_json || '[]', b.errors_json || '[]', b.duration_ms, b.generated_at, now());
    sheetSync.onActivityLogResult({ ...b, machine_id: machineId, received_at: now() });
    res.json({ ok: true });
});
// ── Timeline Result ───────────────────────────────────────────────────────────
exports.qbAgentRouter.post('/timeline-result', (req, res) => {
    const b = req.body || {};
    const machineId = b.machine_id || req.headers['x-machine-id'];
    ...
    events_json, generated_at, received_at) VALUES (?,?,?,?,?,?,?)`)
        .run(machineId, b.store_code, b.file_id, b.business_date, JSON.stringify(b.events || []), b.generated_at, now());
    sheetSync.onTimelineResult({ ...b, machine_id: machineId, received_at: now() });
    res.json({ ok: true });
});
// ── Sync Result ───────────────────────────────────────────────────────────────
exports.qbAgentRouter.post('/sync-result', (req, res) => {
    const b = req.body || {};
    const machineId = b.machine_id || req.headers['x-machine-id'];
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?)`)
        .run(machineId, b.store_code, b.file_id, b.business_date, b.status, b.transactions_synced || 0, b.errors_json || '[]', b.result_json || '{}', b.generated_at, now());
    res.json({ ok: true });
});
// ── Error Report ──────────────────────────────────────────────────────────────
exports.qbAgentRouter.post('/error', (req, res) => {
    const b = req.body || {};
    const machineId = b.machine_id || req.headers['x-machine-id'];
    ...
    VALUES (?,?,?,?,?,?,?,?,?)`)
        .run(machineId, b.store_code, b.severity, b.component, b.message, b.exception, b.context_json || '{}', b.occurred_at, now());
    sheetSync.onError({ ...b, machine_id: machineId, received_at: now() });
    res.json({ ok: true });
});
// ── QB Files ──────────────────────────────────────────────────────────────────
exports.qbAgentRouter.post('/qb-files', (req, res) => {
    const b = req.body || {};
    const machineId = b.machine_id || req.headers['x-machine-id'];
    const files = b.files || [];
    const ts = now();
    for (const f of files) {
      ...
      ON CONFLICT(machine_id, file_id) DO UPDATE SET
        ...
        next_sync_at=excluded.next_sync_at, last_error=excluded.last_error, updated_at=excluded.updated_at`)
            .run(machineId, f.file_id, f.store_code, f.company_file_path, f.expected_company_name, f.enabled ? 1 : 0, f.last_status, f.last_synced_at, f.next_sync_at, f.last_error, ts, ts);
    }
    res.json({ ok: true, upserted: files.length });
});
// ── Sync Cycle ────────────────────────────────────────────────────────────────
exports.qbAgentRouter.post('/sync-cycle', (req, res) => {
    const b = req.body || {};
    const machineId = b.machine_id || req.headers['x-machine-id'];
    ...
    ON CONFLICT(cycle_id) DO UPDATE SET
      ...
      error_count=excluded.error_count, next_cycle_at=excluded.next_cycle_at`)
        .run(cycleId, machineId, b.started_at, b.finished_at, b.total_files || 0, b.pass_count || 0, b.warning_count || 0, b.error_count || 0, b.next_cycle_at, now());
    sheetSync.onSyncCycle({ ...b, cycle_id: cycleId, machine_id: machineId });
    res.json({ ok: true, cycle_id: cycleId });
});
// ── Machines list ─────────────────────────────────────────────────────────────
exports.qbAgentRouter.get('/machines', (_req, res) => {
    const rows = db.prepare('SELECT * FROM machines ORDER BY last_seen_at DESC').all();
    res.json({ machines: rows });
});
// ── Status ────────────────────────────────────────────────────────────────────
exports.qbAgentRouter.get('/status', (_req, res) => {
    const machines = db.prepare('SELECT COUNT(*) as c FROM machines').get();
    const online = db.prepare("SELECT COUNT(*) as c FROM machines WHERE status='online'").get();
    const pending_cmds = db.prepare("SELECT COUNT(*) as c FROM commands WHERE status='pending'").get();
...
// ── QB Files list ─────────────────────────────────────────────────────────────
exports.qbAgentRouter.get('/qb-files', (req, res) => {
    const { machine_id } = req.query;
    const rows = machine_id
        ? db.prepare('SELECT * FROM qb_files WHERE machine_id = ? ORDER BY file_id').all(machine_id)
        : db.prepare('SELECT * FROM qb_files ORDER BY machine_id, file_id').all();
    res.json({ files: rows });
});
// ── Sync Cycles list ──────────────────────────────────────────────────────────
exports.qbAgentRouter.get('/sync-cycles', (req, res) => {
    const { machine_id } = req.query;
    const rows = machine_id
        ? db.prepare('SELECT * FROM sync_cycles WHERE machine_id = ? ORDER BY started_at DESC LIMIT 50').all(machine_id)
        : db.prepare('SELECT * FROM sync_cycles ORDER BY started_at DESC LIMIT 100').all();
    res.json({ cycles: rows });
});
// ── Commands: poll ────────────────────────────────────────────────────────────
exports.qbAgentRouter.get('/commands', (req, res) => {
    const { machine_id } = req.query;
    if (!machine_id) {
        res.status(400).json({ error: 'machine_id required' });
        return;
    }
    const rows = db.prepare("SELECT * FROM commands WHERE machine_id=? AND status='pending' ORDER BY created_at ASC LIMIT 20").all(machine_id);
    res.json({ commands: rows });
});
// ── Commands: create ──────────────────────────────────────────────────────────
exports.qbAgentRouter.post('/commands', (req, res) => {
    const b = req.body || {};
    if (!b.machine_id || !b.command_type) {
        res.status(400).json({ error: 'machine_id and command_type required' });
        return;
    }
    const commandId = cid();
    db.prepare(`INSERT INTO commands (command_id, machine_id, command_type, payload_json, status, created_at)
    VALUES (?,?,?,?,?,?)`)
        .run(commandId, b.machine_id, b.command_type, JSON.stringify(b.payload || {}), 'pending', now());
    res.json({ ok: true, command_id: commandId });
});
// ── Commands: ack ─────────────────────────────────────────────────────────────
exports.qbAgentRouter.post('/commands/:command_id/ack', (req, res) => {
...
// ── Commands: result ──────────────────────────────────────────────────────────
exports.qbAgentRouter.post('/commands/:command_id/result', (req, res) => {
    const { command_id } = req.params;
    const b = req.body || {};
...
exports.qbAgentRouter.get('/recent-activity', (_req, res) => {
