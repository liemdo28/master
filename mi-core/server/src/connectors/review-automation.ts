/**
 * Review Response Automation
 * Drafts AI responses to Google Business + DoorDash reviews.
 * Requires human approval before posting (approval gate).
 */

import https from 'https';
import * as path from 'path';
import * as fs from 'fs';
import { queueToCeo } from '../services/whatsapp-sender';

const REVIEWS_DIR = path.join(
  process.env.GLOBAL_DIR || 'D:/Project/Master/.local-agent-global',
  'reviews'
);

export interface Review {
  id:           string;
  platform:     'google' | 'doordash' | 'yelp';
  store:        string;
  author:       string;
  rating:       number;    // 1-5
  text:         string;
  date:         string;
  responded:    boolean;
  response_draft?: string;
}

export interface ReviewResponse {
  review_id:  string;
  draft:      string;
  tone:       'professional' | 'friendly' | 'apologetic';
  approved:   boolean;
  posted:     boolean;
  error?:     string;
}

// ── Tone selection based on rating ───────────────────────────────────────────

function selectTone(rating: number): 'professional' | 'friendly' | 'apologetic' {
  if (rating >= 4) return 'friendly';
  if (rating === 3) return 'professional';
  return 'apologetic';
}

// ── Draft generation via AI ───────────────────────────────────────────────────

export async function draftResponse(review: Review): Promise<string> {
  const key = process.env.ANTHROPIC_API_KEY || process.env.OPENAI_API_KEY;
  if (!key) return generateTemplateResponse(review);

  const tone = selectTone(review.rating);
  const systemPrompt = `You are a professional restaurant manager for ${review.store}. Write a ${tone} response to a customer review. Be genuine, specific, and concise (2-4 sentences max). Do NOT use generic phrases like "we appreciate your feedback". Address the specific content of their review.`;

  const userPrompt = `Customer review (${review.rating}/5 stars):
"${review.text}"

Write a ${tone} response. Keep it under 100 words.`;

  const body = JSON.stringify({
    model: 'claude-haiku-4-5-20251001',
    max_tokens: 200,
    system: systemPrompt,
    messages: [{ role: 'user', content: userPrompt }],
  });

  return new Promise((resolve) => {
    const req = https.request({
      hostname: 'api.anthropic.com', path: '/v1/messages', method: 'POST',
      headers: { 'x-api-key': key, 'anthropic-version': '2023-06-01', 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) },
    }, (res) => {
      let d = '';
      res.on('data', c => d += c);
      res.on('end', () => {
        try {
          const j = JSON.parse(d);
          resolve(j.content?.[0]?.text || generateTemplateResponse(review));
        } catch { resolve(generateTemplateResponse(review)); }
      });
    });
    req.on('error', () => resolve(generateTemplateResponse(review)));
    req.write(body); req.end();
  });
}

function generateTemplateResponse(review: Review): string {
  if (review.rating >= 4) {
    return `Thank you so much for the wonderful review! We're so glad you enjoyed your experience at ${review.store}. We look forward to seeing you again soon!`;
  }
  if (review.rating === 3) {
    return `Thank you for taking the time to share your feedback about ${review.store}. We appreciate your honest review and will use it to improve your next visit.`;
  }
  return `We sincerely apologize for the experience you had at ${review.store}. This is not the standard we hold ourselves to. Please contact us directly so we can make this right for you.`;
}

// ── Post response to Google Business Profile ──────────────────────────────────

export async function postGoogleResponse(reviewId: string, locationId: string, reply: string): Promise<boolean> {
  const token = process.env.GOOGLE_ACCESS_TOKEN;
  if (!token) return false;

  const body = JSON.stringify({ comment: reply });
  return new Promise((resolve) => {
    const req = https.request({
      hostname: 'mybusiness.googleapis.com',
      path: `/v4/${locationId}/reviews/${reviewId}/reply`,
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
        'Content-Length': Buffer.byteLength(body),
      },
    }, (res) => {
      res.resume();
      res.on('end', () => resolve(res.statusCode === 200));
    });
    req.on('error', () => resolve(false));
    req.write(body); req.end();
  });
}

// ── Full automation pipeline ──────────────────────────────────────────────────

export async function processReviewBatch(reviews: Review[]): Promise<ReviewResponse[]> {
  fs.mkdirSync(REVIEWS_DIR, { recursive: true });

  const responses: ReviewResponse[] = [];

  for (const review of reviews) {
    if (review.responded) continue;

    const draft = await draftResponse(review);
    const tone  = selectTone(review.rating);

    const resp: ReviewResponse = {
      review_id: review.id,
      draft,
      tone,
      approved: false,
      posted:   false,
    };

    // Save draft for approval
    const fname = path.join(REVIEWS_DIR, `${review.id}-draft.json`);
    fs.writeFileSync(fname, JSON.stringify({ review, response: resp }, null, 2));

    // Push to CEO for approval
    const stars = '⭐'.repeat(review.rating);
    const msg = [
      `📝 *Review Response Draft — ${review.store}*`,
      `Platform: ${review.platform} | Rating: ${stars} (${review.rating}/5)`,
      `Reviewer: ${review.author} | Date: ${review.date.slice(0, 10)}`,
      ``,
      `*Review:*`,
      `"${review.text.slice(0, 200)}"`,
      ``,
      `*Draft Response (${tone}):*`,
      `"${draft}"`,
      ``,
      `Reply:`,
      `• \`approve-review ${review.id}\` — post this response`,
      `• \`edit-review ${review.id} <new text>\` — edit then post`,
      `• \`skip-review ${review.id}\` — skip this review`,
    ].join('\n');

    queueToCeo(msg);
    responses.push(resp);
  }

  return responses;
}

// ── Load saved review drafts ──────────────────────────────────────────────────

export function getPendingReviewDrafts(): { review: Review; response: ReviewResponse }[] {
  if (!fs.existsSync(REVIEWS_DIR)) return [];
  return fs.readdirSync(REVIEWS_DIR)
    .filter(f => f.endsWith('-draft.json'))
    .map(f => {
      try { return JSON.parse(fs.readFileSync(path.join(REVIEWS_DIR, f), 'utf8')); }
      catch { return null; }
    })
    .filter(Boolean)
    .filter(d => !d.response.approved);
}
