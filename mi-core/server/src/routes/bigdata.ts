...
bigdataRouter.get('/connectors/quickbooks/status', async (_req: Request, res: Response) => {
  const agentUrl = process.env.QB_AGENT_URL || '';
  const apiKey   = process.env.QB_API_KEY   || '';
  if (!agentUrl || !apiKey) {
    return res.json({ connected: false, note: 'QB_AGENT_URL / QB_API_KEY not set in .env' });
  }
  try {
    ...
    const data = await r.json() as Record<string, unknown>;
    return res.json({ connected: r.ok, status: r.status, agent: data });
  } catch (e) {
    return res.status(500).json({ connected: false, error: String(e) });
  }
});
