...
export function executePlan(objectiveId:string):ExecutionPlan|null{
  ...
  obj.evidenceCount=obj.plan.tasks.reduce((s:any,t:any)=>s+t.evidence.length,0); saveObjective(obj); log(objectiveId,'PLAN_EXECUTION_COMPLETED',obj.plan.progress); return obj.plan;
}
...
export function trackProgress(objectiveId:string):ExecutionPlan|null{ const obj=loadObjective(objectiveId); if(!obj?.plan)return null; const total=obj.plan.tasks.length; const completed=obj.plan.tasks.filter((t:any)=>t.status==='completed'||t.status==='qa-passed').length; const failed=obj.plan.tasks.filter((t:any)=>t.status==='failed'||t.status==='qa-failed').length; const inProgress=obj.plan.tasks.filter((t:any)=>t.status==='in-progress').length; const pending=obj.plan.tasks.filter((t:any)=>t.status==='pending').length; obj.plan.progress={totalTasks:total,completedTasks:completed,failedTasks:failed,inProgressTasks:inProgress,pendingTasks:pending,percentComplete:total?Math.round((completed/total)*100):0}; saveObjective(obj); return obj.plan; }

export function verifyCompletion(objectiveId:string):{completed:boolean;evidenceCount:number;overallScore:number;failedTasks:string[]}{ const obj=loadObjective(objectiveId); if(!obj?.plan)return {completed:false,evidenceCount:0,overallScore:0,failedTasks:[]}; const completed=obj.plan.tasks.filter((t:any)=>t.status==='completed'||t.status==='qa-passed').length; const failed=obj.plan.tasks.filter((t:any)=>t.status==='failed'||t.status==='qa-failed'); const evidenceCount=obj.plan.tasks.reduce((s:any,t:any)=>s+t.evidence.length,0); const overallScore=obj.plan.tasks.length?Math.round((completed/obj.plan.tasks.length)*100):0; log(objectiveId,'VERIFICATION_COMPLETE',{completed,total:obj.plan.tasks.length,score:overallScore}); return {completed:completed===obj.plan.tasks.length,evidenceCount,overallScore,failedTasks:failed.map((t:any)=>t.title)}; }
