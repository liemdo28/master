...
const PACK_CONTENT: Record<string, Array<{ title: string; content: string }>> = {
  'restaurant-ops': [
...
function getInstalledPacks(): Pack[] {
  try {
    return JSON.parse(fs.readFileSync(path.join(PACKS_DIR, 'installed.json'), 'utf-8'));
  } catch {
    return PACK_DEFINITIONS.map(p => ({ ...p, installed: false }));
  }
}

function saveInstalledPacks(packs: Pack[]) {
  fs.mkdirSync(PACKS_DIR, { recursive: true });
  fs.writeFileSync(path.join(PACKS_DIR, 'installed.json'), JSON.stringify(packs, null, 2));
}
...
export function installPack(packId: string): { ok: boolean; docs_added: number; message: string } {
  ...
  for (const doc of content) {
    ...
    fs.writeFileSync(path.join(packDir, filename), doc.content, 'utf-8');
    written++;
  }
...
export function uninstallPack(packId: string): { ok: boolean } {
  const packDir = path.join(PACKS_DIR, packId);
  if (fs.existsSync(packDir)) fs.rmSync(packDir, { recursive: true });
  const packs = getInstalledPacks();
  const idx = packs.findIndex(p => p.pack_id === packId);
  if (idx >= 0) packs[idx].installed = false;
  saveInstalledPacks(packs);
  return { ok: true };
}
