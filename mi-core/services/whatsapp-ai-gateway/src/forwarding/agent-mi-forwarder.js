...
function httpPost(targetUrl, payload, timeoutMs = TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    ...
    const apiKey = payload && payload.api_key ? String(payload.api_key) : '';
    const options = {
...
async function forward(payload, clientId, commandPrefix) {
  ...
  const maxRetries = clientId === 'mi-core' ? 0 : MAX_RETRIES;
  ...
  await auditLog.record({
    ...
    detail: 'Forward failed after ' + (maxRetries + 1) + ' attempts: ' + lastError,
    success: false,
  });

  return {
    ok: false,
    reply: clientId === 'mi-core' ? null : safeErrorReply(clientId),
    request_message_id: payload.message_id || '',
    error: lastError,
