...
function installOutboundSendGuard(targetClient) {
  if (!targetClient || targetClient.__miOutboundSendGuardInstalled) return targetClient;
  if (typeof targetClient.sendMessage !== 'function') return targetClient;
  const originalSendMessage = targetClient.sendMessage.bind(targetClient);
  targetClient.sendMessage = async (to, content, options) => {
    const text = typeof content === 'string'
      ? content
      : (options && typeof options.caption === 'string' ? options.caption : '');
    if (isBlockedUserFacingText(text)) {
      log.warn('[MESSAGE_FLOW] blocked_banned_session_sendMessage', {
        to,
        preview: String(text || '').replace(/\s+/g, ' ').slice(0, 160),
      });
      writeDiagnostics('blocked_banned_session_sendMessage', {
        to,
        preview: String(text || '').replace(/\s+/g, ' ').slice(0, 160),
      });
      return null;
  ...
  Object.defineProperty(targetClient, '__miOutboundSendGuardInstalled', {
    value: true,
    enumerable: false,
    configurable: false,
  });
  writeDiagnostics('outbound_send_guard_installed');
  return targetClient;
}
...
function getClient() { return installOutboundSendGuard(client); }
