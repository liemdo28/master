...
/**
 * Ensure required clients exist using already-configured environment keys.
 * This does not expose raw keys and does not generate replacement keys.
 */
async function ensureConfiguredClients() {
  const envByClient = {
    'agent-coding': process.env.AGENT_CODING_API_KEY || '',
    'mi-core': process.env.MI_CORE_API_KEY || '',
  };
  ...
  for (const client of DEFAULT_CLIENTS) {
    ...
    const rawKey = envByClient[client.clientId] || '';
    if (!rawKey) {
      log.warn(`Default client "${client.clientId}" missing and no env key configured`);
      continue;
    }
    ...
    await run(
      `INSERT INTO api_keys (client_id, api_key_hash, key_prefix, allowed_commands, callback_url, status, created_at, last_used_at, rate_limit, permissions, description)
       VALUES (?, ?, ?, ?, ?, 'active', ?, NULL, ?, ?, ?)`,
      [client.clientId, hashKey(rawKey), prefix, client.allowedCommands, client.callbackUrl, now, client.rateLimit, client.permissions, client.description]
    );

    await auditLog.record({
      clientId: client.clientId,
      action: auditLog.ACTIONS.KEY_CREATED,
      detail: `API key registered from environment with prefix ${prefix}`,
      success: true,
    });

    log.info(`Default client "${client.clientId}" registered from environment — key prefix: ${prefix}`);
  }
}
