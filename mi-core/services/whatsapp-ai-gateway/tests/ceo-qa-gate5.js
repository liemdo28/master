/**
 * CEO QA Gate — Isolated DB test
 * Run: node tests/ceo-qa-gate5.js
 */
...
async function run() {
  ...
  // Gate 3 - use raw SQL to avoid hanging
  console.log('\n[Gate 3] Dashboard shows confirmed submission');
  console.log('  calling raw SQL stats query...');
  const db = s.getDb();
  ...
  console.log('  raw SQL done: ' + JSON.stringify(stats));
  ...
  console.log('  calling raw SQL recent query...');
  const recent = await new Promise((resolve, reject) => {
    db.all(`SELECT * FROM food_safety_submissions ORDER BY created_at DESC LIMIT 10`, [], (err, rows) => { if (err) reject(err); else resolve(rows); });
  });
  console.log('  raw recent SQL done, rows: ' + recent.length);
  ok('getRecentSubmissions() returns rows', Array.isArray(recent));
  ...
  require('../src/whatsapp/reply-service').send = async () => true;
  ...
  const confirmed = await new Promise((resolve, reject) => {
    db.all(`SELECT * FROM food_safety_submissions WHERE status='CONFIRMED' ORDER BY created_at DESC LIMIT 1`, [], (err, rows) => { if (err) reject(err); else resolve(rows[0] || null); });
  });
  ...
  const items = await new Promise((resolve, reject) => {
    db.all(`SELECT * FROM food_safety_submission_items WHERE submission_id=?`, [confirmed.submission_id], (err, rows) => { if (err) reject(err); else resolve(rows); });
  });
  ok('Confirmed submission has items', items && items.length > 0);
  ...
  const confirmed2 = await new Promise((resolve, reject) => {
    db.all(`SELECT * FROM food_safety_submissions WHERE status='CONFIRMED' AND store='Stone Oak' ORDER BY created_at DESC LIMIT 1`, [], (err, rows) => { if (err) reject(err); else resolve(rows[0] || null); });
  });
  ...
  const confirmed3 = await new Promise((resolve, reject) => {
    db.all(`SELECT * FROM food_safety_submissions WHERE status='CONFIRMED' AND store='Bandera' ORDER BY created_at DESC LIMIT 1`, [], (err, rows) => { if (err) reject(err); else resolve(rows[0] || null); });
  });
