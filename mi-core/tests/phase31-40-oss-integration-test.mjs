...
console.log('PHASE 31-40 OSS INTEGRATION TEST\n');
...
check('oss-runtime workers >= 29',()=>assert.ok(typeof summary.workers==='number'&&summary.workers>=29));
const PHASE_OSS={31:'SupplyChainOS',32:'LegalComplianceOS',33:'ProductInnovationOS',34:'FleetTransportOS',35:'FraudRiskOS',36:'CustomerLoyaltyOS',37:'PartnerChannelOS',38:'FinanceAccountingOS',39:'DataWarehouseOS',40:'AutonomousOpsOS'};
for(const [phase,ossName] of Object.entries(PHASE_OSS)){
  ...
  check(`Phase ${phase} worker has name`,()=>assert.ok(worker&&worker.worker&&worker.worker.name));
