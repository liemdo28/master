...
export type MiMode = 'ceo' | 'developer' | 'personal' | 'restaurant' | 'finance' | 'health' | 'focus' | 'emergency';

export type IntentType =
  | 'memory_save' | 'memory_forget' | 'profile_view' | 'health_view'
  | 'chat' | 'command' | 'reminder'
  | 'briefing' | 'project_issues' | 'project_search' | 'pending_approvals'
  | 'visibility_daily' | 'visibility_overdue' | 'visibility_email'
  | 'visibility_calendar' | 'visibility_dashboard' | 'visibility_health'
...
export function parseIntent(text: string): ParsedIntent {
  ...
  // Detect mode
  let mode: MiMode = 'personal';
  if (/doanh thu|tài chính|finance|revenue|profit|invoice|quickbooks/i.test(t)) mode = 'finance';
  else if (/project|bug|fix|deploy|code|github|git|dev|build/i.test(t)) mode = 'developer';
  else if (/ceo|chiến lược|strategy|executive|board|report|weekly/i.test(t)) mode = 'ceo';
  else if (/rawsushi|bakudan|menu|restaurant|nhà hàng|kitchen|order/i.test(t)) mode = 'restaurant';
  else if (/sức khỏe|uống nước|nghỉ ngơi|sleep|health|workout|vitamin/i.test(t)) mode = 'health';
  else if (/khẩn cấp|urgent|emergency|down|crash|incident/i.test(t)) mode = 'emergency';

  return { type: 'chat', mode, raw };
}

// Build system prompt based on mode + owner profile
export function buildSystemPrompt(mode: MiMode): string {
  const profile = ownerProfile.getAll();
  const name = (profile.profile as Record<string, string>)?.preferred_name || 'anh';
  const lang = (profile.preferences as Record<string, string>)?.language || 'vi';
  const responseStyle = (profile.preferences as Record<string, string>)?.response_style || 'short, clear, actionable';
...
Work style: ${JSON.stringify(profile.work_style)}`;
  ...
  const modeContexts: Record<MiMode, string> = {
