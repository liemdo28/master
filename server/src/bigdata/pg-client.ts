 ...
 * Wraps pg driver with connection pooling.
 * Falls back gracefully when Postgres is not configured.
 * Env vars: PG_HOST, PG_PORT, PG_DATABASE, PG_USER, PG_PASSWORD
 *
 * Setup:
 *   docker run -d --name mi-postgres -e POSTGRES_PASSWORD=micore -p 5432:5432 pgvector/pgvector:pg16
 *   Then add to .env: PG_HOST=localhost PG_PORT=5432 PG_DATABASE=micore PG_USER=postgres PG_PASSWORD=micore
...
=======
  try {
    ...
    pool = new Pool({
      ...
      database: process.env.PG_DATABASE || 'micore',
    ...
    pool.on('error', (_err: Error) => { /* silent */ });
    pgAvailable = true;
    return pool;
  } catch {
    pgAvailable = false;
...
export function isPgAvailable(): boolean {
  return pgAvailable && !!process.env.PG_HOST;
}
...
export async function ensurePgSchema(): Promise<boolean> {
  ...
  try {
    ...
    // Engineering tasks (replaces SQLite for scale)
    await p.query(`
      CREATE TABLE IF NOT EXISTS engineering_tasks_pg (
        id              TEXT PRIMARY KEY,
        objective       TEXT NOT NULL,
        project         TEXT DEFAULT 'mi-core',
        selected_model  TEXT,
    ...
    console.warn('[PgClient] Schema setup failed:', e.message);
    return false;
  }
}
...
export async function vectorSearch(embedding: number[], topK = 5): Promise<{
...
export async function insertEmbedding(
  ...
  try {
    await pgQuery(
      `INSERT INTO embeddings (source, content, embedding, metadata) VALUES ($1, $2, $3::vector, $4)`,
