...
import { searchLocalFiles, formatFileResults, checkFileBlocked } from './file-search';
...
export function isDailyWorkAction(message: string): boolean {
...
export async function handleDailyWorkAction(message: string): Promise<ActionResult | null> {
  const msg = message;
  ...
  if (/raw là|bakudan ở|store nào|ở đâu.*(?:raw|bakudan)|(?:raw|bakudan).*ở đâu|bakudan là|raw sushi bar là/i.test(msg)) {
    const answer = answerStoreQuery(msg);
    if (answer) {
  ...
  if (/^(?:tìm|find|search)\s+(?:file|report|báo cáo)/i.test(msg.trim()) &&
    ...
    const files = searchLocalFiles(query, 5);
    const reply = formatFileResults(files as Parameters<typeof formatFileResults>[0], query);
  ...
  if (/tìm.*rồi gửi|find.*then send|gửi file.*cho|send.*file.*to/i.test(msg)) {
    ...
    const person = resolvePerson(recipientRaw);
    const recipientName = person?.name || recipientRaw;

    // Find file
    const files = searchLocalFiles(fileQuery, 5);
    ...
    // Single file found
    const file = files[0];
    const blockCheck = checkFileBlocked(file.path);
  ...
  if (/upload.*(?:drive|lên drive)|lên.*drive|put.*drive/i.test(msg)) {
    ...
    const files = searchLocalFiles(query, 3);
    if (!Array.isArray(files) || files.length === 0) {
      return {
        type: 'upload-drive',
