...
async function run() {
  ...
  assert.ok(!('evidencePath' in inspectBody), 'HTTP response must not expose an absolute evidence path');
  assert.strictEqual(inspectBody.evidenceId, 'step-1-command');
  assert.strictEqual(inspectBody.relativePath, `${task.id}/step-1-command.json`);
  log(`task completed via HTTP, exitCode=${inspectBody.exitCode}`);

  const evidenceRes = await fetch(`${app1.baseUrl}/tasks/${task.id}/evidence/${inspectBody.evidenceId}`);
  assert.strictEqual(evidenceRes.status, 200);
  const evidenceBody = await evidenceRes.json();
  assert.strictEqual(evidenceBody.exitCode, 0);

  const evidenceTraversalRes = await fetch(`${app1.baseUrl}/tasks/${task.id}/evidence/${encodeURIComponent('../tasks.db')}`);
  assert.strictEqual(evidenceTraversalRes.status, 400, 'evidence path traversal must be rejected');
