...
const FORBIDDEN_PATTERNS: ForbiddenPattern[] = [
  ...
  },
  {
    ...
    patterns: [
      /\b(buy|sell|trade|transfer|withdraw|deposit)\s+([\w.$]+\s+)?(of\s+(the\s+)?)?(stock|shares?|crypto|bitcoin|funds?|money)\b/i,
  ...
  },
  {
    ...
    patterns: [
      /\bdeploy\s+(this\s+|the\s+|it\s+)?(to\s+production|now|the\s+app)?\b.*\bdeploy\b|\bdeploy\s+(it|this|now)\b/i,
      /\bmerge\s+and\s+deploy\b/i,
      /\bpush\s+to\s+production\b/i,
    ],
  },
  {
    label: 'FORBIDDEN_BROWSER_WRITE',
    patterns: [
      /\b(click|submit|fill\s+(in|out))\s+(the\s+)?(button|form|field)\b/i,
  ...
  },
  {
    ...
    patterns: [
      /\b(approve|authorize)\s+(it|this|that|everything|all)\s+(automatically|from\s+now\s+on|without\s+asking)\b/i,
