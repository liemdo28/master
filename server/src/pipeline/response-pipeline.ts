...
export async function runPipeline(input: PipelineInput): Promise<PipelineOutput> {
  ...
  if (/^(create|tạo|giao|add)\s+(task|việc)/i.test(message.trim())) {
    ...
    if (Array.isArray(draft)) {
      const lines = [
        `✅ ${draft.length} task drafts created (split by store):`,
        ...draft.map(d => `  • "${d.title}"${d.store ? ` — Store: ${d.store}` : ''}${d.assignee ? ` — Assignee: ${d.assignee}` : ''}${d.due_date ? ` — Due: ${d.due_date}` : ''}`),
        '',
        `→ Approval #${draft[0]?.approval_id} required before creating`,
        `[Approve] [Edit] [Reject]`,
      ];
      liveDataParts.push(`[Task Drafts Created]\n${lines.join('\n')}`);
    } else {
