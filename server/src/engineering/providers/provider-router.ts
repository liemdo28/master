 ...
 * Uses https.request directly, with no SDK dependencies.
 */
...
export async function routeToProvider(req: ProviderRequest): Promise<ProviderResponse> {
  ...
  try {
    switch (provider) {
      case 'openai':
        return await callOpenAI(model, messages, req, start);
      case 'anthropic':
        return await callAnthropic(model, messages, req, start);
      case 'gemini':
        return await callGemini(model, req, start);
      case 'deepseek':
        return await callDeepSeek(model, messages, req, start);
      case 'xai':
        return await callXAI(model, messages, req, start);
      default:
