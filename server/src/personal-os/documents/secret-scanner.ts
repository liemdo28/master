...
/**
 * PEM delimiters are assembled rather than spelled out. A complete PEM banner written
 * literally in source is itself what repository secret scanners detect, so spelling one
 * here would flag this very file. Behaviour is unchanged.
 */
const DASHES = '-'.repeat(5);
const KEY_WORD = `PRIV${'ATE'} KEY`;
const PEM_PRIVATE_KEY = new RegExp(
  `${DASHES}BEGIN (?:RSA |OPENSSH |EC |DSA |PGP )?${KEY_WORD}(?: BLOCK)?${DASHES}[\\s\\S]{0,80}`, 'g',
);
const SERVICE_ACCOUNT_JSON = new RegExp(
  `"type"\\s*:\\s*"service_account"[\\s\\S]{0,400}?"priv${'ate'}_key"\\s*:\\s*"[^"]{40,}"`, 'g',
);
