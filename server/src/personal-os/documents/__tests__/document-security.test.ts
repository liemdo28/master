...
async function run() {
  ...
  // PEM banners are assembled too, and the body is plain filler rather than a base64 key
  // prefix, so no key-shaped material exists anywhere in this file's source text.
  const dashes = '-'.repeat(5);
  const keyWord = `PRIV${'ATE'} KEY`;
  const secrets: Array<[string, string]> = [
    ['PRIVATE_KEY', `${dashes}BEGIN RSA ${keyWord}${dashes}\n${filler}${filler}\n${dashes}END RSA ${keyWord}${dashes}`],
    ...
    ['SERVICE_ACCOUNT', `{"type":"service_account","project_id":"p","priv${'ate'}_key":"${dashes}BEGIN ${keyWord}${dashes}${filler}${filler}"}`],
  ];
