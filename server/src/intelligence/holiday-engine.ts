...
export interface HolidayInfo {
  ...
  type: 'federal' | 'california' | 'local' | 'cultural' | 'commercial';
  business_impact: 'high' | 'medium' | 'low';
  traffic_effect: 'up' | 'down' | 'mixed';
...
// ── Fixed-date holidays ────────────────────────────────────────────────────
const FIXED_HOLIDAYS: Omit<HolidayInfo, 'date'>[] & { month: number; day: number }[] = [
  {
...
function computedHolidays(year: number): HolidayInfo[] {
  ...
  const lunarNewYear: Record<number, string> = {
...
// ── Main API ──────────────────────────────────────────────────────────────
export function getHolidaysForDateRange(startDate: Date, endDate: Date): HolidayInfo[] {
  ...
  for (const y of years) {
    ...
    for (const h of FIXED_HOLIDAYS) {
      ...
      all.push({ ...h, date } as HolidayInfo);
    }
    // Computed
...
export function getWeekContext(referenceDate?: Date): WeekContext {
...
export function getUpcomingHolidays(days = 30, referenceDate?: Date): HolidayInfo[] {
  const now = referenceDate || new Date();
...
// ── Summary builders ──────────────────────────────────────────────────────
function buildWeekSummary(weekHolidays: HolidayInfo[], upcoming: HolidayInfo[], now: Date): string {
  const lines: string[] = [];
...
function buildMarketingSuggestions(upcoming: HolidayInfo[]): string[] {
  if (!upcoming.length) return ['No major holidays this week. Good time for evergreen content or SEO posts.'];
...
export function getHolidayContextString(referenceDate?: Date): string {
