"use strict";
// ============================================================
// Agent OS - Control Plane - Kill Switch Routes
// Emergency stop for tasks, workers, and processes
// ============================================================
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const taskQueue_1 = require("../services/taskQueue");
const database_1 = require("../services/database");
const wsManager_1 = require("../services/wsManager");
const journal_1 = require("../services/journal");
const router = (0, express_1.Router)();
/**
 * POST /api/kill/task/:id — Kill a running task
 */
router.post('/task/:id', (req, res) => {
    const taskId = req.params.id;
    const task = taskQueue_1.taskQueue.getTask(taskId);
    if (!task) {
        return res.status(404).json({ error: 'Task not found' });
    }
    if (task.status !== 'running' && task.status !== 'pending') {
        return res.status(400).json({ error: `Cannot kill task in status: ${task.status}` });
    }
    // Update task status
    taskQueue_1.taskQueue.updateTaskStatus(taskId, 'cancelled', 'Killed by CEO via kill switch');
    // Notify worker via WebSocket
    wsManager_1.wsManager.broadcast({
        type: 'task_kill',
        payload: { taskId, workerId: task.workerId, reason: 'CEO kill switch' },
        timestamp: new Date().toISOString(),
    });
    (0, journal_1.writeJournalEvent)({ type: 'kill_task', taskId, actor: 'ceo', data: { workerId: task.workerId } });
    res.json({
        success: true,
        message: `Task #${taskId.slice(0, 8)} killed.`,
        taskId,
    });
});
/**
 * POST /api/kill/worker/:id — Stop a worker
 */
router.post('/worker/:id', (req, res) => {
    const workerId = req.params.id;
    const worker = database_1.workerDb.findById(workerId);
    if (!worker) {
        return res.status(404).json({ error: 'Worker not found' });
    }
    // Update worker status
    database_1.workerDb.updateStatus(workerId, 'stopped');
    // Send stop signal via WebSocket
    wsManager_1.wsManager.broadcast({
        type: 'worker_stop',
        payload: { workerId, reason: 'CEO kill switch' },
        timestamp: new Date().toISOString(),
    });
    (0, journal_1.writeJournalEvent)({ type: 'kill_worker', actor: 'ceo', data: { workerId, workerName: worker.name } });
    res.json({
        success: true,
        message: `Worker ${worker.name} stopped.`,
        workerId,
    });
});
/**
 * POST /api/kill/all — Emergency stop all workers and tasks
 */
router.post('/all', (req, res) => {
    // Kill all running tasks
    const tasks = taskQueue_1.taskQueue.getTasks();
    const killed = [];
    for (const task of tasks) {
        if (task.status === 'running' || task.status === 'pending') {
            taskQueue_1.taskQueue.updateTaskStatus(task.id, 'cancelled', 'Emergency stop by CEO');
            killed.push(task.id);
        }
    }
    // Stop all workers
    const workers = database_1.workerDb.findAll();
    for (const worker of workers) {
        if (worker.status === 'online' || worker.status === 'busy') {
            database_1.workerDb.updateStatus(worker.id, 'stopped');
        }
    }
    // Broadcast emergency stop
    wsManager_1.wsManager.broadcast({
        type: 'emergency_stop',
        payload: { killedTasks: killed.length, stoppedWorkers: workers.length, reason: 'CEO emergency stop' },
        timestamp: new Date().toISOString(),
    });
    (0, journal_1.writeJournalEvent)({ type: 'emergency_stop', actor: 'ceo', data: { killedTasks: killed, stoppedWorkers: workers.map((w) => w.id) } });
    res.json({
        success: true,
        message: `Emergency stop: ${killed.length} tasks killed, ${workers.length} workers stopped.`,
        killedTasks: killed.length,
        stoppedWorkers: workers.length,
    });
});
exports.default = router;
//# sourceMappingURL=kill.js.map