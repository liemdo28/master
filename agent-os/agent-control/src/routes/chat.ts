...
import { parseIntent, describeIntent, getUnsupportedCommandMessage } from '../services/intentEngine';
import { createCommanderPlan, formatCommanderPlan } from '../services/agentCommander';
import { approveTask, denyTask, getPendingApprovals } from '../services/approvalEngine';
...
router.post('/', (req: Request, res: Response) => {
  ...
  const commanderPlan = createCommanderPlan(trimmed, intent);
  ...
  if (!intent.supported && !commanderPlan.recognized) {
    const sysMsg = {
      ...
      content: getUnsupportedCommandMessage(intent.suggestions),
      timestamp: new Date().toISOString(),
      intent,
      commanderPlan,
    };
  ...
  if (commanderPlan.taskType === 'query' || (intent.supported && intent.type === 'query')) {
    ...
    const sysMsg = {
      id: uuid(),
      role: 'system' as const,
      content: `${formatCommanderPlan(commanderPlan)}\n\n${queryResponse}`,
      timestamp: new Date().toISOString(),
      intent,
      commanderPlan,
    };
  ...
  const task = taskQueue.createTask({
    type: commanderPlan.taskType as TaskType,
...
router.post('/approve', (req: Request, res: Response) => {
  ...
  const task = tasks.find((t: any) => t.id.startsWith(taskId));
  if (!task) {
    return res.status(404).json({ error: 'Task not found' });
  }

  const approval = approveTask(task.id, 'ceo');
  if (!approval) {
...
router.post('/deny', (req: Request, res: Response) => {
  ...
  const approval = denyTask(task.id, 'ceo', reason);
  if (!approval) {
    return res.status(400).json({ error: 'No pending approval for this task' });
  }

  taskQueue.updateTaskStatus(task.id, 'cancelled');
