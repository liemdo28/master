...
export async function handleCline(
  task: any,
  streamLog: (level: string, msg: string, data?: any) => void
): Promise<void> {
  const payload = task.payload as ClineTaskPayload;

  if (!payload?.prompt) throw new Error('Cline task requires a prompt');
...
interface Tool {
  name: 'bash' | 'write_file' | 'read_file';
  description: string;
  input_schema: object;
}
...
function explainBlockedCommand(cmd: string): string | null {
...
function buildProjectContext(projectPath: string, contextFiles?: string[]): string {
  const lines: string[] = [];
