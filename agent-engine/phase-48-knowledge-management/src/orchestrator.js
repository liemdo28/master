// phase-48-knowledge-management - Phase 48. Modules: Internal knowledge base + SOP + training library
...
export class KnowledgeManagementOS {
  constructor(opts={}) {
    this.registry=new JsonStore('ph48-reg',opts);
    this.signals=new JsonStore('ph48-sig',opts);
    this.alerts=new JsonStore('ph48-alr',opts);
  }
  register(s){
    const r={id:makeId('S48'),timestamp:Date.now(),signal:s.signal||s,status:'open',approvalRequired:!!(s.requiresApproval),assignedTo:s.assignedTo||null};
  ...
  dashboard(){const s=this.signals.all();const a=this.alerts.all();const open=s.filter(x=>x.status==='open').length;const esc=s.filter(x=>x.status==='escalated').length;const crit=a.filter(x=>x.level==='critical'&&!x.acknowledged).length;return{phase:48,cls:'KnowledgeManagementOS',total:s.length,open,escalated:esc,criticalAlerts:crit,status:crit>0?'CRITICAL':esc>3?'BUSY':open>5?'ACTIVE':'NORMAL'};}
}

export class KnowledgeManagementOSOrchestrator{constructor(opts={}){this.os=new KnowledgeManagementOS(opts);}dashboard(){return this.os.dashboard();}}
export default KnowledgeManagementOSOrchestrator;
