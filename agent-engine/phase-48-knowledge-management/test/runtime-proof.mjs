// Phase 48 runtime proof - phase-48-knowledge-management
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 48 -- KnowledgeManagementOS :: RUNTIME PROOF');
const {KnowledgeManagementOS}=await import(`../src/orchestrator.js`);
const os=new KnowledgeManagementOS();
const s1=os.register({signal:'test-signal',requiresApproval:false});
check('KnowledgeManagementOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(dash.phase,48));
