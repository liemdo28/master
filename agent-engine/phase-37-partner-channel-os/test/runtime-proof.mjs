// Phase 37 runtime proof - phase-37-partner-channel-os
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 37 -- PartnerChannelOS :: RUNTIME PROOF');
const {PartnerChannelOS}=await import(`../src/orchestrator.js`);
const os=new PartnerChannelOS();
const s1=os.register({signal:'test',requiresApproval:false});
check('PartnerChannelOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(os.dashboard().phase,37));
