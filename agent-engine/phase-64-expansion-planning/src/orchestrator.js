// phase-64-expansion-planning - Phase 64. Modules: Expansion planning engine
...
export class ExpansionPlanningOS {
  constructor(opts={}) {
    this.registry=new JsonStore('ph64-reg',opts);
    this.signals=new JsonStore('ph64-sig',opts);
    this.alerts=new JsonStore('ph64-alr',opts);
  }
  register(s){
    const r={id:makeId('S64'),timestamp:Date.now(),signal:s.signal||s,status:'open',approvalRequired:!!(s.requiresApproval),assignedTo:s.assignedTo||null};
  ...
  dashboard(){const s=this.signals.all();const a=this.alerts.all();const open=s.filter(x=>x.status==='open').length;const esc=s.filter(x=>x.status==='escalated').length;const crit=a.filter(x=>x.level==='critical'&&!x.acknowledged).length;return{phase:64,cls:'ExpansionPlanningOS',total:s.length,open,escalated:esc,criticalAlerts:crit,status:crit>0?'CRITICAL':esc>3?'BUSY':open>5?'ACTIVE':'NORMAL'};}
}

export class ExpansionPlanningOSOrchestrator{constructor(opts={}){this.os=new ExpansionPlanningOS(opts);}dashboard(){return this.os.dashboard();}}
export default ExpansionPlanningOSOrchestrator;
