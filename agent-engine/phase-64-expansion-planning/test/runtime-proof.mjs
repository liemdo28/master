// Phase 64 runtime proof - phase-64-expansion-planning
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 64 -- ExpansionPlanningOS :: RUNTIME PROOF');
const {ExpansionPlanningOS}=await import(`../src/orchestrator.js`);
const os=new ExpansionPlanningOS();
const s1=os.register({signal:'test-signal',requiresApproval:false});
check('ExpansionPlanningOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(dash.phase,64));
