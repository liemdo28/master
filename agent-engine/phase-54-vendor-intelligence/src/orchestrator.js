// phase-54-vendor-intelligence - Phase 54. Modules: Vendor intelligence - suppliers/contracts/costs/performance
...
export class VendorIntelligenceOS {
  constructor(opts={}) {
    this.registry=new JsonStore('ph54-reg',opts);
    this.signals=new JsonStore('ph54-sig',opts);
    this.alerts=new JsonStore('ph54-alr',opts);
  }
  register(s){
    const r={id:makeId('S54'),timestamp:Date.now(),signal:s.signal||s,status:'open',approvalRequired:!!(s.requiresApproval),assignedTo:s.assignedTo||null};
  ...
  dashboard(){const s=this.signals.all();const a=this.alerts.all();const open=s.filter(x=>x.status==='open').length;const esc=s.filter(x=>x.status==='escalated').length;const crit=a.filter(x=>x.level==='critical'&&!x.acknowledged).length;return{phase:54,cls:'VendorIntelligenceOS',total:s.length,open,escalated:esc,criticalAlerts:crit,status:crit>0?'CRITICAL':esc>3?'BUSY':open>5?'ACTIVE':'NORMAL'};}
}

export class VendorIntelligenceOSOrchestrator{constructor(opts={}){this.os=new VendorIntelligenceOS(opts);}dashboard(){return this.os.dashboard();}}
