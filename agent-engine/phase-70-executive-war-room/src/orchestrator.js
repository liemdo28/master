// phase-70-executive-war-room - Phase 70. Modules: Executive war room
...
export class ExecutiveWarRoomOS {
  constructor(opts={}) {
    this.registry=new JsonStore('ph70-reg',opts);
    this.signals=new JsonStore('ph70-sig',opts);
    this.alerts=new JsonStore('ph70-alr',opts);
  }
  register(s){
    const r={id:makeId('S70'),timestamp:Date.now(),signal:s.signal||s,status:'open',approvalRequired:!!(s.requiresApproval),assignedTo:s.assignedTo||null};
  ...
  dashboard(){const s=this.signals.all();const a=this.alerts.all();const open=s.filter(x=>x.status==='open').length;const esc=s.filter(x=>x.status==='escalated').length;const crit=a.filter(x=>x.level==='critical'&&!x.acknowledged).length;return{phase:70,cls:'ExecutiveWarRoomOS',total:s.length,open,escalated:esc,criticalAlerts:crit,status:crit>0?'CRITICAL':esc>3?'BUSY':open>5?'ACTIVE':'NORMAL'};}
}

export class ExecutiveWarRoomOSOrchestrator{constructor(opts={}){this.os=new ExecutiveWarRoomOS(opts);}dashboard(){return this.os.dashboard();}}
export default ExecutiveWarRoomOSOrchestrator;
