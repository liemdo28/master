// Phase 70 runtime proof - phase-70-executive-war-room
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 70 -- ExecutiveWarRoomOS :: RUNTIME PROOF');
const {ExecutiveWarRoomOS}=await import(`../src/orchestrator.js`);
const os=new ExecutiveWarRoomOS();
const s1=os.register({signal:'test-signal',requiresApproval:false});
check('ExecutiveWarRoomOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(dash.phase,70));
