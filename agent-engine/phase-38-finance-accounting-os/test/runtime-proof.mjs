// Phase 38 runtime proof - phase-38-finance-accounting-os
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 38 -- FinanceAccountingOS :: RUNTIME PROOF');
const {FinanceAccountingOS}=await import(`../src/orchestrator.js`);
const os=new FinanceAccountingOS();
const s1=os.register({signal:'test',requiresApproval:false});
check('FinanceAccountingOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(os.dashboard().phase,38));
