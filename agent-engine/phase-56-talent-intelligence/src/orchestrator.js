// phase-56-talent-intelligence - Phase 56. Modules: Talent intelligence - skills + performance + capacity + retention risk
...
export class TalentIntelligenceOS {
  constructor(opts={}) {
    this.registry=new JsonStore('ph56-reg',opts);
    this.signals=new JsonStore('ph56-sig',opts);
    this.alerts=new JsonStore('ph56-alr',opts);
  }
  register(s){
    const r={id:makeId('S56'),timestamp:Date.now(),signal:s.signal||s,status:'open',approvalRequired:!!(s.requiresApproval),assignedTo:s.assignedTo||null};
  ...
  dashboard(){const s=this.signals.all();const a=this.alerts.all();const open=s.filter(x=>x.status==='open').length;const esc=s.filter(x=>x.status==='escalated').length;const crit=a.filter(x=>x.level==='critical'&&!x.acknowledged).length;return{phase:56,cls:'TalentIntelligenceOS',total:s.length,open,escalated:esc,criticalAlerts:crit,status:crit>0?'CRITICAL':esc>3?'BUSY':open>5?'ACTIVE':'NORMAL'};}
}

export class TalentIntelligenceOSOrchestrator{constructor(opts={}){this.os=new TalentIntelligenceOS(opts);}dashboard(){return this.os.dashboard();}}
