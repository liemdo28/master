// Phase 80 runtime proof - phase-80-executive-decision-lab
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 80 -- ExecutiveDecisionLabOS :: RUNTIME PROOF');
const {ExecutiveDecisionLabOS}=await import(`../src/orchestrator.js`);
const os=new ExecutiveDecisionLabOS();
const s1=os.register({signal:'test-signal',requiresApproval:false});
check('ExecutiveDecisionLabOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(dash.phase,80));
