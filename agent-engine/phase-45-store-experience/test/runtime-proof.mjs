// Phase 45 runtime proof - phase-45-store-experience
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 45 -- StoreExperienceOS :: RUNTIME PROOF');
const {StoreExperienceOS}=await import(`../src/orchestrator.js`);
const os=new StoreExperienceOS();
const s1=os.register({signal:'test-signal',requiresApproval:false});
check('StoreExperienceOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(dash.phase,45));
