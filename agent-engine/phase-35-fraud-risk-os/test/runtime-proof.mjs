// Phase 35 runtime proof - phase-35-fraud-risk-os
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 35 -- FraudRiskOS :: RUNTIME PROOF');
const {FraudRiskOS}=await import(`../src/orchestrator.js`);
const os=new FraudRiskOS();
const s1=os.register({signal:'test',requiresApproval:false});
check('FraudRiskOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(os.dashboard().phase,35));
