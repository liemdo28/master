// Phase 39 runtime proof - phase-39-data-warehouse-os
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 39 -- DataWarehouseOS :: RUNTIME PROOF');
const {DataWarehouseOS}=await import(`../src/orchestrator.js`);
const os=new DataWarehouseOS();
const s1=os.register({signal:'test',requiresApproval:false});
check('DataWarehouseOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(os.dashboard().phase,39));
