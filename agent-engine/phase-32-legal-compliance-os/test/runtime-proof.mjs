// Phase 32 runtime proof - phase-32-legal-compliance-os
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 32 -- LegalComplianceOS :: RUNTIME PROOF');
const {LegalComplianceOS}=await import(`../src/orchestrator.js`);
const os=new LegalComplianceOS();
const s1=os.register({signal:'test',requiresApproval:false});
check('LegalComplianceOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(os.dashboard().phase,32));
