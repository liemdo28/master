// Phase 36 runtime proof - phase-36-customer-loyalty-os
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 36 -- CustomerLoyaltyOS :: RUNTIME PROOF');
const {CustomerLoyaltyOS}=await import(`../src/orchestrator.js`);
const os=new CustomerLoyaltyOS();
const s1=os.register({signal:'test',requiresApproval:false});
check('CustomerLoyaltyOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(os.dashboard().phase,36));
