// phase-95-ceo-shadow-model - Phase 95. Modules: CEO shadow model
...
export class CEOShadowModelOS {
  constructor(opts={}) {
    this.registry=new JsonStore('ph95-reg',opts);
    this.signals=new JsonStore('ph95-sig',opts);
    this.alerts=new JsonStore('ph95-alr',opts);
  }
  register(s){
    const r={id:makeId('S95'),timestamp:Date.now(),signal:s.signal||s,status:'open',approvalRequired:!!(s.requiresApproval),assignedTo:s.assignedTo||null};
  ...
  dashboard(){const s=this.signals.all();const a=this.alerts.all();const open=s.filter(x=>x.status==='open').length;const esc=s.filter(x=>x.status==='escalated').length;const crit=a.filter(x=>x.level==='critical'&&!x.acknowledged).length;return{phase:95,cls:'CEOShadowModelOS',total:s.length,open,escalated:esc,criticalAlerts:crit,status:crit>0?'CRITICAL':esc>3?'BUSY':open>5?'ACTIVE':'NORMAL'};}
}

export class CEOShadowModelOSOrchestrator{constructor(opts={}){this.os=new CEOShadowModelOS(opts);}dashboard(){return this.os.dashboard();}}
export default CEOShadowModelOSOrchestrator;
