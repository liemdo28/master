// Phase 95 runtime proof - phase-95-ceo-shadow-model
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 95 -- CEOShadowModelOS :: RUNTIME PROOF');
const {CEOShadowModelOS}=await import(`../src/orchestrator.js`);
const os=new CEOShadowModelOS();
const s1=os.register({signal:'test-signal',requiresApproval:false});
check('CEOShadowModelOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(dash.phase,95));
