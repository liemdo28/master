// Phase 31 runtime proof - phase-31-supply-chain-os
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 31 -- SupplyChainOS :: RUNTIME PROOF');
const {SupplyChainOS}=await import(`../src/orchestrator.js`);
const os=new SupplyChainOS();
const s1=os.register({signal:'test',requiresApproval:false});
check('SupplyChainOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(os.dashboard().phase,31));
