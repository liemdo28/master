// Phase 90 runtime proof - phase-90-autonomous-company-management
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 90 -- AutonomousCompanyManagementOS :: RUNTIME PROOF');
const {AutonomousCompanyManagementOS}=await import(`../src/orchestrator.js`);
const os=new AutonomousCompanyManagementOS();
const s1=os.register({signal:'test-signal',requiresApproval:false});
check('AutonomousCompanyManagementOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(dash.phase,90));
