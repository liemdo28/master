// Phase 44 runtime proof - phase-44-supplier-management
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 44 -- SupplierManagementOS :: RUNTIME PROOF');
const {SupplierManagementOS}=await import(`../src/orchestrator.js`);
const os=new SupplierManagementOS();
const s1=os.register({signal:'test-signal',requiresApproval:false});
check('SupplierManagementOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(dash.phase,44));
