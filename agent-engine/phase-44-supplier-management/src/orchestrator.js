// phase-44-supplier-management - Phase 44. Modules: Supplier onboarding + scoring + relationship management
...
export class SupplierManagementOS {
  constructor(opts={}) {
    this.registry=new JsonStore('ph44-reg',opts);
    this.signals=new JsonStore('ph44-sig',opts);
    this.alerts=new JsonStore('ph44-alr',opts);
  }
  register(s){
    const r={id:makeId('S44'),timestamp:Date.now(),signal:s.signal||s,status:'open',approvalRequired:!!(s.requiresApproval),assignedTo:s.assignedTo||null};
  ...
  dashboard(){const s=this.signals.all();const a=this.alerts.all();const open=s.filter(x=>x.status==='open').length;const esc=s.filter(x=>x.status==='escalated').length;const crit=a.filter(x=>x.level==='critical'&&!x.acknowledged).length;return{phase:44,cls:'SupplierManagementOS',total:s.length,open,escalated:esc,criticalAlerts:crit,status:crit>0?'CRITICAL':esc>3?'BUSY':open>5?'ACTIVE':'NORMAL'};}
}

export class SupplierManagementOSOrchestrator{constructor(opts={}){this.os=new SupplierManagementOS(opts);}dashboard(){return this.os.dashboard();}}
export default SupplierManagementOSOrchestrator;
