// phase-69-reputation-defense - Phase 69. Modules: Reputation defense system
...
export class ReputationDefenseOS {
  constructor(opts={}) {
    this.registry=new JsonStore('ph69-reg',opts);
    this.signals=new JsonStore('ph69-sig',opts);
    this.alerts=new JsonStore('ph69-alr',opts);
  }
  register(s){
    const r={id:makeId('S69'),timestamp:Date.now(),signal:s.signal||s,status:'open',approvalRequired:!!(s.requiresApproval),assignedTo:s.assignedTo||null};
  ...
  dashboard(){const s=this.signals.all();const a=this.alerts.all();const open=s.filter(x=>x.status==='open').length;const esc=s.filter(x=>x.status==='escalated').length;const crit=a.filter(x=>x.level==='critical'&&!x.acknowledged).length;return{phase:69,cls:'ReputationDefenseOS',total:s.length,open,escalated:esc,criticalAlerts:crit,status:crit>0?'CRITICAL':esc>3?'BUSY':open>5?'ACTIVE':'NORMAL'};}
}

export class ReputationDefenseOSOrchestrator{constructor(opts={}){this.os=new ReputationDefenseOS(opts);}dashboard(){return this.os.dashboard();}}
export default ReputationDefenseOSOrchestrator;
