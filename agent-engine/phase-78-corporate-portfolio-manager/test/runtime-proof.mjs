// Phase 78 runtime proof - phase-78-corporate-portfolio-manager
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 78 -- CorporatePortfolioManagerOS :: RUNTIME PROOF');
const {CorporatePortfolioManagerOS}=await import(`../src/orchestrator.js`);
const os=new CorporatePortfolioManagerOS();
const s1=os.register({signal:'test-signal',requiresApproval:false});
check('CorporatePortfolioManagerOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(dash.phase,78));
