// phase-66-franchise-intelligence - Phase 66. Modules: Franchise intelligence
...
export class FranchiseIntelligenceOS {
  constructor(opts={}) {
    this.registry=new JsonStore('ph66-reg',opts);
    this.signals=new JsonStore('ph66-sig',opts);
    this.alerts=new JsonStore('ph66-alr',opts);
  }
  register(s){
    const r={id:makeId('S66'),timestamp:Date.now(),signal:s.signal||s,status:'open',approvalRequired:!!(s.requiresApproval),assignedTo:s.assignedTo||null};
  ...
  dashboard(){const s=this.signals.all();const a=this.alerts.all();const open=s.filter(x=>x.status==='open').length;const esc=s.filter(x=>x.status==='escalated').length;const crit=a.filter(x=>x.level==='critical'&&!x.acknowledged).length;return{phase:66,cls:'FranchiseIntelligenceOS',total:s.length,open,escalated:esc,criticalAlerts:crit,status:crit>0?'CRITICAL':esc>3?'BUSY':open>5?'ACTIVE':'NORMAL'};}
}

export class FranchiseIntelligenceOSOrchestrator{constructor(opts={}){this.os=new FranchiseIntelligenceOS(opts);}dashboard(){return this.os.dashboard();}}
export default FranchiseIntelligenceOSOrchestrator;
