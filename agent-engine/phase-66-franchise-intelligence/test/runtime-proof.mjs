// Phase 66 runtime proof - phase-66-franchise-intelligence
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 66 -- FranchiseIntelligenceOS :: RUNTIME PROOF');
const {FranchiseIntelligenceOS}=await import(`../src/orchestrator.js`);
const os=new FranchiseIntelligenceOS();
const s1=os.register({signal:'test-signal',requiresApproval:false});
check('FranchiseIntelligenceOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(dash.phase,66));
