// Phase 86 runtime proof - phase-86-self-evolving-agents
import * as assert from 'assert';
let passed=0,failed=0;
const check=(n,f)=>{try{f();passed++;console.log('  PASS: '+n);}catch(e){failed++;console.error('  FAIL: '+n+' -- '+e.message);}};
console.log('PHASE 86 -- SelfEvolvingAgentsOS :: RUNTIME PROOF');
const {SelfEvolvingAgentsOS}=await import(`../src/orchestrator.js`);
const os=new SelfEvolvingAgentsOS();
const s1=os.register({signal:'test-signal',requiresApproval:false});
check('SelfEvolvingAgentsOS registers signal',()=>assert.ok(s1&&s1.id));
...
check('dashboard() phase correct',()=>assert.strictEqual(dash.phase,86));
