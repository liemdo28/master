// phase-63-competitor-intelligence - Phase 63. Modules: Competitor intelligence engine
...
export class CompetitorIntelligenceOS {
  constructor(opts={}) {
    this.registry=new JsonStore('ph63-reg',opts);
    this.signals=new JsonStore('ph63-sig',opts);
    this.alerts=new JsonStore('ph63-alr',opts);
  }
  register(s){
    const r={id:makeId('S63'),timestamp:Date.now(),signal:s.signal||s,status:'open',approvalRequired:!!(s.requiresApproval),assignedTo:s.assignedTo||null};
  ...
  dashboard(){const s=this.signals.all();const a=this.alerts.all();const open=s.filter(x=>x.status==='open').length;const esc=s.filter(x=>x.status==='escalated').length;const crit=a.filter(x=>x.level==='critical'&&!x.acknowledged).length;return{phase:63,cls:'CompetitorIntelligenceOS',total:s.length,open,escalated:esc,criticalAlerts:crit,status:crit>0?'CRITICAL':esc>3?'BUSY':open>5?'ACTIVE':'NORMAL'};}
}

export class CompetitorIntelligenceOSOrchestrator{constructor(opts={}){this.os=new CompetitorIntelligenceOS(opts);}dashboard(){return this.os.dashboard();}}
export default CompetitorIntelligenceOSOrchestrator;
