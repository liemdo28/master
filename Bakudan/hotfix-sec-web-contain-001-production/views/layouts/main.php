...
$unreadCount = $unreadCount ?? (
    (int)$notifModel->getUnreadCount($_SESSION['user_id'])
    + (int)(new TaskNotification())->getUnreadCount($_SESSION['user_id'])
);
...
try {
    ...
    // BUGFIX RC1-001: Admin/manager query was incorrectly filtering by assignee_id,
    // preventing CEO from seeing org-wide overdue/task counts in sidebar badges.
    // Admin branch now returns ALL tasks (no assignee filter).
    // Non-admin branch preserves per-user scope.
    $_sbRow = $_sbAdmin
        ? $_sbDb->fetch(
            "SELECT
                SUM(t.is_completed=0 AND t.status NOT IN ('completed','done','cancelled') AND t.due_date<?) AS overdue,
                SUM(t.is_completed=0 AND t.status NOT IN ('completed','done','cancelled') AND t.due_date=?) AS today
             FROM tasks t",
            [$_sbToday, $_sbToday])
        : $_sbDb->fetch(
            "SELECT
                SUM(t.is_completed=0 AND t.status NOT IN ('completed','done','cancelled') AND t.due_date<? AND t.assignee_id=?) AS overdue,
                SUM(t.is_completed=0 AND t.status NOT IN ('completed','done','cancelled') AND t.due_date=?  AND t.assignee_id=?) AS today
             FROM tasks t",
...
<head>
    ...
    <link rel="stylesheet" href="<?= APP_URL ?>/assets/css/detail-drawer.css">
...
<div class="app-layout">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sb-nav" id="sbNav">
            ...
            <!-- ─── CEO FOCUS ─── -->
            <div class="sb-group">
                <div class="sb-section">CEO FOCUS</div>

                <a href="<?= APP_URL ?>/overview#ceo-today" class="<?= sbCls($cp,['overview','dashboard','ceo']) ?>">
                    <span class="sb-item__icon"><?= tf_icon('target',16) ?></span>
                    <span class="sb-item__label">Today Brief</span>
                ...
                <a href="<?= APP_URL ?>/bills?filter=overdue" class="<?= ($cp==='bills' && ($_GET['filter']??'')==='overdue') ? 'sb-item sb-item--active' : 'sb-item' ?>">
                    ...
                    <span class="sb-item__label">Pay / File</span>
                    <?= sbBadge($sbBadges['payments'] + $sbBadges['filings'], 'payments') ?>
                </a>
                ...
                <a href="<?= APP_URL ?>/my-tasks?filter=overdue" class="<?= ($cp==='my-tasks' && ($_GET['filter']??'')==='overdue') ? 'sb-item sb-item--active' : 'sb-item' ?>">
                    ...
                    <span class="sb-item__label">Follow Up</span>
            ...
            <div class="sb-group">
                ...
                <a href="<?= APP_URL ?>/exception-queue" class="<?= sbCls($cp,['exception-queue']) ?>">
                    <span class="sb-item__icon"><?= tf_icon('alert-triangle',16) ?></span>
                    <span class="sb-item__label">Exception Queue</span>
                </a>

                <a href="<?= APP_URL ?>/task-type-view" class="<?= sbCls($cp,['task-type-view']) ?>">
                    <span class="sb-item__icon"><?= tf_icon('layers',16) ?></span>
                    <span class="sb-item__label">Task Type View</span>
                </a>
                <?php endif; ?>
            ...
            <div class="sb-group">
                ...
                <a href="<?= APP_URL ?>/team-load" class="<?= sbCls($cp,['team-load']) ?>">
            ...
            <div class="sb-group">
                ...
                <?php if (canManage()): /* Overall Store: CEO + Admin + Manager */ ?>
                <a href="<?= APP_URL ?>/overall-store" class="<?= sbCls($cp,['overall-store']) ?>">
                    <span class="sb-item__icon"><?= tf_icon('grid',16) ?></span>
                    <span class="sb-item__label">Overall Store</span>
                </a>
                <?php endif; ?>

                <?php if (canManage()): /* Store Command: admin + CEO + manager */ ?>
                ...
                <?php if (canAdmin()): /* All Stores: admin + CEO */ ?>
                ...
                <?php if (canManage()): /* Store Health: admin + CEO + manager */ ?>
            ...
            <div class="sb-group">
                ...
                <?php if (isAdmin()): ?>
                <a href="<?= APP_URL ?>/admin/data-freshness" class="<?= sbCls($cp,['admin-data-freshness']) ?>">
                    <span class="sb-item__icon"><?= tf_icon('activity',16) ?></span>
                    <span class="sb-item__label">Data Freshness</span>
                </a>
                <?php endif; ?>

                <?php if (isAdmin()): /* Vendors + Budget: admin only, not CEO */ ?>
            ...
            <div class="sb-group">
                ...
                <a href="<?= APP_URL ?>/penalties" class="<?= sbCls($cp,['my-penalties']) ?>">
                    <span class="sb-item__icon"><?= tf_icon('alert-circle',16) ?></span>
                    <span class="sb-item__label">My Penalties</span>
                </a>
                <?php if (canManage() && !canAdmin()): ?>
                <a href="<?= APP_URL ?>/manager/penalties" class="<?= sbCls($cp,['manager-penalties']) ?>">
                    <span class="sb-item__icon"><?= tf_icon('users',16) ?></span>
                    <span class="sb-item__label">Team Penalties</span>
                </a>
                <?php endif; ?>
            ...
            <div class="sb-group">
                ...
                <a href="<?= APP_URL ?>/ceo/accountability" class="<?= sbCls($cp,['ceo-accountability']) ?>">
                    <span class="sb-item__icon"><?= tf_icon('shield',16) ?></span>
                    <span class="sb-item__label">Accountability</span>
                </a>
            </div>
            <?php endif; ?>

            <!-- ─── ADMIN (collapsed, strict admin only — CEO excluded) ─── -->
            <?php if (isAdmin()): ?>
            <div class="sb-group sb-group--system" id="sbSystemGroup">
                ...
                <div class="sb-group-items hidden" id="sbSystemItems">
                    ...
                    <a href="<?= APP_URL ?>/admin/penalties" class="<?= sbCls($cp,['admin-penalties']) ?>">
                        <span class="sb-item__icon"><?= tf_icon('alert-octagon',16) ?></span>
                        <span class="sb-item__label">Penalty Dashboard</span>
                    </a>
                    <a href="<?= APP_URL ?>/admin/penalty-rules" class="<?= sbCls($cp,['penalty-rules']) ?>">
                        <span class="sb-item__icon"><?= tf_icon('sliders',16) ?></span>
                        <span class="sb-item__label">Penalty Rules</span>
                    </a>
                    <a href="<?= APP_URL ?>/admin/penalty" class="<?= sbCls($cp,['penalty-config']) ?>">
                        <span class="sb-item__icon"><?= tf_icon('user-x',16) ?></span>
                        <span class="sb-item__label">User Penalty Config</span>
                    </a>
                    <a href="<?= APP_URL ?>/admin/task-audit" class="<?= sbCls($cp,['task-audit-review']) ?>">
                        <span class="sb-item__icon"><?= tf_icon('shield-alert',16) ?></span>
                        <span class="sb-item__label">Task Audit</span>
                    </a>
                    <?php
                    try {
    ...
    <main class="main-content">
        <header class="page-header">
            ...
            <div class="header-actions">
                ...
                <button type="button" class="gs-trigger" data-gs-trigger data-testid="header-search-trigger" aria-label="Search">
                    <span>🔍</span>
                    <span>Search...</span>
                ...
                <div class="create-new-wrap">
                    <button class="create-new-btn" data-action="toggle-create-dropdown" data-testid="header-create-new-btn">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 5v14M5 12h14"/></svg>
                        <?= e(t('create.new')) ?>
                    </button>
                    <div class="create-new-dropdown" id="createNewDropdown">
                        <a href="#" data-action="open-create-task" data-testid="create-task-link">
                            <span class="dd-icon" style="background:var(--blue-bg);color:var(--blue)"><?= tf_icon('check-square', 16) ?></span>
                            <?= e(t('create.task')) ?>
                        </a>
                        <a href="<?= APP_URL ?>/projects/create" data-action="close-create-dropdown" data-testid="create-project-link">
                            <span class="dd-icon" style="background:var(--green-bg);color:var(--green)"><?= tf_icon('folder', 16) ?></span>
                            <?= e(t('create.project')) ?>
                        </a>
                        <div class="dd-sep"></div>
                        <a href="<?= APP_URL ?>/bills" data-action="close-create-dropdown" data-testid="create-bill-link">
                            <span class="dd-icon" style="background:var(--amber-bg);color:var(--amber)"><?= tf_icon('receipt', 16) ?></span>
                            <?= e(t('create.bill')) ?>
                        </a>
                        <?php if (canAdmin()): ?>
                        <a href="<?= APP_URL ?>/admin/stores" onclick="closeCreateDropdown()" data-testid="create-store-link">
                            <span class="dd-icon" style="background:var(--purple-bg);color:var(--purple)"><?= tf_icon('store', 16) ?></span>
                            <?= e(t('create.store')) ?>
                        </a>
                        <a href="<?= APP_URL ?>/admin/vendors" data-action="close-create-dropdown" data-testid="create-vendor-link">
                ...
                <a href="<?= APP_URL ?>/ai-import" class="btn btn-outline btn-sm" data-testid="header-ai-import-btn"><?= tf_icon('robot', 16) ?> <?= e(t('ai_import.nav_btn')) ?></a>
                <?= $headerActions ?? '' ?>

                <!-- Notification Bell -->
                <div style="position:relative" id="notifWrap">
                    <button class="notif-btn" data-action="toggle-notif" data-testid="header-notif-bell" aria-label="<?= e(t('header.notifications')) ?>" aria-haspopup="true">
                        <?= tf_icon('bell') ?>
                    ...
                    <div class="notif-dropdown" id="notifDropdown">
                        <div class="notif-dropdown-header">
                            ...
                            <button class="btn btn-sm btn-secondary" data-action="mark-notif-read" data-testid="notif-mark-all-read"><?= e(t('header.read_all')) ?></button>
                        </div>
                        <div id="notifList">
...
<div class="modal-overlay ct-modal" id="createTaskModal" data-testid="create-task-modal">
    <div class="modal-box">
        ...
        <form action="<?= APP_URL ?>/tasks" method="POST" id="quickTaskForm" data-testid="create-task-form">
        <div class="modal-body">
            ...
            <div class="ct-form-grid">
                ...
                <div class="form-group" style="margin:0">
                    <label style="font-size:11px;color:#71717a;font-weight:600">Category</label>
                    <select name="task_categories[]" class="form-control" multiple size="5">
                        ...
                        <option value="store_operation">Store Operation</option>
            ...
            <div class="ct-approval-wrap">
                ...
                <div class="ct-approval-fields" id="ctApprovalFields" style="display:none">
                    ...
                    <div class="form-group" style="margin:0">
                        <label style="font-size:11px;color:#71717a;font-weight:600">Reviewer Deadline</label>
                        <input type="date" name="reviewer_due_date" class="form-control" style="font-size:12px">
                    </div>
                </div>
...
var CT_SECTIONS = <?= json_encode($qtSections, JSON_HEX_TAG | JSON_UNESCAPED_UNICODE) ?>;
...
// Duplicate task check on quick-task form submit
(function(){
    ...
    form.addEventListener('submit', async function(e){
        ...
        check.append('assignee_id', fd.get('assignee_id') || '');
        check.append('due_date',    fd.get('due_date') || '');
        check.append('category',    fd.get('task_category') || '');
        let res;
        try { res = await fetch(window.APP_URL + '/api/tasks/check-duplicate', {method:'POST', body:check}).then(r=>r.json()); }
        catch { form.submit(); return; }
        if (!res || !res.duplicate) { form.submit(); return; }
        const m = res.match || {};
        const msg = `⚠️ Possible duplicate task detected (${res.score}% match)\n\n` +
            `Existing: "${m.title || ''}"\n` +
            `Assignee: ${m.assignee || '—'}  ·  Due: ${m.due_date || '—'}\n\n` +
            `Save anyway?`;
...
<script src="<?= APP_URL ?>/assets/js/detail-drawer.js"></script>
