...
<div class="td-main">
    ...
    <!-- Tab nav -->
    <div class="td-tabs">
        <button class="td-tab active" onclick="tdTab(event,'general')"><?= tf_icon('file-text', 13) ?> General</button>
        <?php if ($approvalRequired || canAdmin()): ?>
        <button class="td-tab" onclick="tdTab(event,'workflow')"><?= tf_icon('git-branch', 13) ?> Workflow</button>
        <?php endif; ?>
        <button class="td-tab" onclick="tdTab(event,'evidence')"><?= tf_icon('paperclip', 13) ?> Evidence
            <?php if (!empty($attachments)): ?><span class="td-tab-badge"><?= count($attachments) ?></span><?php endif; ?>
        </button>
        <button class="td-tab" onclick="tdTab(event,'comments')"><?= tf_icon('message-square', 13) ?> Comments
        ...
        <button class="td-tab" onclick="tdTab(event,'history')"><?= tf_icon('clock', 13) ?> History</button>
    </div>
    ...
    <div id="td-comments" class="td-pane">
        <div class="task-tabs" style="margin-bottom:14px">
            <button class="task-tab-btn active" data-tab="comm-comments" onclick="switchTaskTab(event,'comm-comments')"><?= tf_icon('hash', 14) ?> Comments (<?= count($taskComments ?? []) ?>)</button>
            <button class="task-tab-btn" data-tab="comm-reviewer" onclick="switchTaskTab(event,'comm-reviewer')"><?= tf_icon('bookmark', 14) ?> Reviewer Notes<?= !empty($pendingCount)?' ('.$pendingCount.')':'' ?></button>
            <button class="task-tab-btn" data-tab="comm-approval" onclick="switchTaskTab(event,'comm-approval')"><?= tf_icon('check-square', 14) ?> Approval Notes (<?= count($approvalNotes ?? []) ?>)</button>
        </div>
    ...
    <div id="td-history" class="td-pane">
        ...
        <div style="display:flex;flex-direction:column;gap:6px">
            ...
            <a href="<?= APP_URL ?>/tasks/<?= $child['id'] ?>" class="reschedule-link" style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:9px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);color:#b8c0cc;text-decoration:none">
                ...
                <?php if ($child['due_date']): ?><span class="text-muted text-sm">→ <?= date('d/m/Y', strtotime($child['due_date'])) ?></span><?php endif; ?>
...
<div class="td-sidebar">
    ...
    <div class="td-sidebar-card">
        ...
        <?php $daysLeft = (int)ceil((strtotime($task['due_date']) - time()) / 86400); ?>
        <div style="font-size:20px;font-weight:800;color:<?= $daysLeft < 0?'#f87171':($daysLeft<=3?'#fbbf24':'#d4dde8') ?>"><?= date('d/m/Y', strtotime($task['due_date'])) ?></div>
    ...
    <div class="modal-box" style="width:400px">
        ...
        <form method="POST" action="<?= APP_URL ?>/tasks/<?= $task['id'] ?>/reschedule">
            <div class="modal-body">
                ...
                <?php if ($task['due_date']): ?><p class="text-sm" style="margin-bottom:12px">Current: <strong><?= date('d/m/Y', strtotime($task['due_date'])) ?></strong></p><?php endif; ?>
...
<div class="modal-overlay" id="extendModal">
    <div class="modal-box" style="width:440px">
        ...
        <div class="modal-body">
            ...
            <p class="text-sm" style="margin-bottom:4px">Current: <strong><?= date('d/m/Y', strtotime($task['due_date'])) ?></strong></p>
            <p class="text-sm" style="margin-bottom:14px">Self-extend up to: <strong><?= date('d/m/Y', strtotime($task['due_date'].' +3 days')) ?></strong></p>
...
function tdTab(e, tab) {
    ...
    document.querySelectorAll('.td-pane').forEach(function(p){ p.classList.remove('active'); });
    if (e && e.currentTarget) e.currentTarget.classList.add('active');
    var pane = document.getElementById('td-' + tab);
    if (pane) pane.classList.add('active');
}
function switchTaskTab(e, tab) {
    var parent = e.currentTarget.closest('.card-body') || document.querySelector('.td-pane.active');
