...
export default function EditUserPage() {
  ...
  const [form, setForm] = useState({ name: '', role: 'viewer', brand_id: '', store_id: '', password: '', is_active: true });
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/users`).then((r) => r.json()).then((d) => {
      const user = (d.data ?? []).find((u: any) => String(u.id) === id);
      if (user) setForm({ name: user.name, role: user.role, brand_id: String(user.brand_id ?? ''), store_id: String(user.store_id ?? ''), password: '', is_active: user.is_active });
    });
  }, [id]);
