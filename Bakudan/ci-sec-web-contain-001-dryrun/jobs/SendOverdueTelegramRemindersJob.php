<?php
/**
 * SendOverdueTelegramRemindersJob — send overdue reminders to all linked Telegram users.
 *
 * Schedule: 9:00 AM daily (after due-today):
 *   0 9 * * * curl -s "https://dashboard.bakudanramen.com/api/telegram/jobs/overdue?secret=CRON_SECRET" > /dev/null 2>&1
 */
class SendOverdueTelegramRemindersJob {
    ...
    public function run(): array {
        $sent = $this->notifService->sendOverdueReminders();
        return [
            'job'   => 'overdue_reminders',
            'date'  => function_exists('app_today') ? app_today() : date('Y-m-d'),
            'sent'  => $sent,
