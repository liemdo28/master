 ...
 * Fetches and responds to Google Business Profile reviews — headless, no browser.
 * Auth: OAuth 2.0 refresh token (one-time setup via scripts/reauth.mjs).
 */
...
const LOCATIONS: Array<{ key: string; name: string; locationId: string }> = [
  ...
  },
  {
    ...
    locationId: process.env.GOOGLE_LOCATION_ID_BAKUDAN_BANDERA ?? "",
  },
  {
    key: "bakudan-rim",
    name: "Bakudan Ramen (The Rim)",
    locationId: process.env.GOOGLE_LOCATION_ID_BAKUDAN_RIM ?? "",
  },
  {
    key: "bakudan-stone-oak",
    name: "Bakudan Ramen (Stone Oak)",
    locationId: process.env.GOOGLE_LOCATION_ID_BAKUDAN_STONE_OAK ?? "",
  },
...
class ReviewServer {
  ...
  private tools(): Tool[] {
    ...
    return [
      ...
      },
      {
        ...
        description: "List all recent reviews (responded and unresponded) with optional filters.",
        inputSchema: {
          type: "object",
          properties: {
            location: {
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            response_text: { type: "string", description: "The response text to post" },
            approved_by_owner: {
              type: "boolean",
              default: false,
      ...
      },
      {
        ...
        description:
          ...
          "and follow-up tracking. Use this for ALL negative reviews instead of posting directly.",
        inputSchema: {
          type: "object",
          required: [
            "location",
          ...
          properties: {
            ...
            draft_response: {
              type: "string",
              description: "Empathetic draft for manager to review",
            },
          },
        },
  ...
  private wire() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (req) => {
      ...
      try {
        switch (name) {
          case "list_new_reviews":
            return await this.listNewReviews(args);
          case "list_all_reviews":
            return await this.listAllReviews(args);
          case "post_review_response":
            return await this.postResponse(args);
          case "save_pending_review":
            return await this.savePendingReview(args);
  ...
  private async listNewReviews(args: any) {
    ...
    for (const loc of targets) {
      ...
      const fresh = reviews.filter(
        (r) =>
          !this.tracker.hasResponded(r.id) &&
          !r.reply &&
          (!sinceDate || r.date.slice(0, 10) >= sinceDate)
      );
  ...
  private async savePendingReview(args: any) {
    const { location, review_id, reviewer_name, stars, review_date, review_text, draft_response } =
      args;
    const loc = LOCATIONS.find((l) => l.key === location);
    ...
    if (store.pending.find((r: any) => r.review_id === review_id)) {
      return {
        content: [
          { type: "text", text: `Already queued: ${reviewer_name} (${stars}★) at ${loc.name}` },
        ],
      };
    }
    ...
    const mdLine =
      `\n## ${loc.name} — ${reviewer_name} ${"⭐".repeat(stars)} — ${review_date}\n` +
      `**Review:** ${review_text}\n` +
    ...
    return {
      content: [
        {
          type: "text",
          text: `✅ Queued for manager notification: ${reviewer_name} (${stars}★) at ${loc.name}`,
        },
      ],
    };
  }
  ...
  async run() {
    ...
    console.error("Review Management MCP Server started (headless GBP API mode).");
  }
}
