/* Auto-generated smoke test for seo-citation-agent */
...
async function run() {
  const { server } = createAgent({
    agentId: 'seo-citation-agent',
    version: '1.0.0',
    port: 0,
    agentDir: __dirname,
    runAudit: async ({ saveReport, db }) => {
      saveReport({ agentId: 'seo-citation-agent', type: 'smoke', payload: { summary: 'smoke' }, db });
  ...
  const s = await get('/status'); assert.strictEqual(s.status, 200); assert.strictEqual(s.body.agent, 'seo-citation-agent');
  ...
  server.close();
  console.log('seo-citation-agent: ALL TESTS PASSED');
}
