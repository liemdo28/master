...
// Always re-sync brands.json/locations.json from the canonical mi-core copy
// before validating, so a stale/hand-edited root copy is auto-corrected
// rather than silently validated as-is. See sync-from-canonical.js and
// docs/seo-control-center/SEO_PATH_CONSOLIDATION.md for the full rationale.
try {
  const syncFromCanonical = require(path.join(root, 'SEO', 'shared', 'config', 'sync-from-canonical.js'));
  const syncReport = syncFromCanonical({ silent: true });
  for (const entry of syncReport) {
    if (entry.action === 'error') {
      console.log(`[validate-brand-config] WARNING: pre-sync failed for ${entry.file}: ${entry.error}`);
    } else if (entry.action !== 'unchanged') {
      console.log(`[validate-brand-config] pre-sync: ${entry.file} ${entry.action} from canonical before validation`);
    }
  }
} catch (e) {
  console.log(`[validate-brand-config] WARNING: could not run pre-sync step: ${e.message}`);
}
