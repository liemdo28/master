/* Auto-generated smoke test for seo-analytics-agent */
...
async function run() {
  const { server } = createAgent({
    agentId: 'seo-analytics-agent',
    version: '1.0.0',
    port: 0,
    agentDir: __dirname,
    runAudit: async ({ saveReport, db }) => {
      saveReport({ agentId: 'seo-analytics-agent', type: 'smoke', payload: { summary: 'smoke' }, db });
  ...
  const s = await get('/status'); assert.strictEqual(s.status, 200); assert.strictEqual(s.body.agent, 'seo-analytics-agent');
  ...
  server.close();
  console.log('seo-analytics-agent: ALL TESTS PASSED');
}
