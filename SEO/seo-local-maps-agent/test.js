/* Auto-generated smoke test for seo-local-maps-agent */
...
async function run() {
  const { server } = createAgent({
    agentId: 'seo-local-maps-agent',
    version: '1.0.0',
    port: 0,
    agentDir: __dirname,
    runAudit: async ({ saveReport, db }) => {
      saveReport({ agentId: 'seo-local-maps-agent', type: 'smoke', payload: { summary: 'smoke' }, db });
  ...
  const s = await get('/status'); assert.strictEqual(s.status, 200); assert.strictEqual(s.body.agent, 'seo-local-maps-agent');
  ...
  server.close();
  console.log('seo-local-maps-agent: ALL TESTS PASSED');
}
