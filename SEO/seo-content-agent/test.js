/* Auto-generated smoke test for seo-content-agent */
...
async function run() {
  const { server } = createAgent({
    ...
    runAudit: async ({ saveReport, db }) => {
      saveReport({ agentId: 'seo-content-agent', type: 'smoke', payload: { summary: 'smoke' }, db });
  ...
  const s = await get('/status'); assert.strictEqual(s.status, 200); assert.strictEqual(s.body.agent, 'seo-content-agent');
  ...
  server.close();
  console.log('seo-content-agent: ALL TESTS PASSED');
}
