/* Auto-generated smoke test for seo-technical-agent */
...
async function run() {
  const { server } = createAgent({
    ...
    runAudit: async ({ saveReport, db }) => {
      saveReport({ agentId: 'seo-technical-agent', type: 'smoke', payload: { summary: 'smoke' }, db });
  ...
  const s = await get('/status'); assert.strictEqual(s.status, 200); assert.strictEqual(s.body.agent, 'seo-technical-agent');
