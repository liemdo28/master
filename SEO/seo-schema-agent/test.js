/* Auto-generated smoke test for seo-schema-agent */
...
async function run() {
  const { server } = createAgent({
    agentId: 'seo-schema-agent',
    version: '1.0.0',
    port: 0,
    agentDir: __dirname,
    runAudit: async ({ saveReport, db }) => {
      saveReport({ agentId: 'seo-schema-agent', type: 'smoke', payload: { summary: 'smoke' }, db });
  ...
  const s = await get('/status'); assert.strictEqual(s.status, 200); assert.strictEqual(s.body.agent, 'seo-schema-agent');
  ...
  server.close();
  console.log('seo-schema-agent: ALL TESTS PASSED');
}
