...
function getRuntimeHealth() {
  ...
  try { waStatus = getStatus().status || 'unknown'; whatsappReady = waStatus === 'ready'; } catch (_) {}
...
app.post('/api/whatsapp/reset-session', async (req, res) => {
  res.set('Cache-Control', 'no-store');
  try {
    const status = await whatsappSession.resetSession();
    res.json({ ok: true, status });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message, status: whatsappSession.getStatus() });
  }
...
app.get('/api/stats', async (req, res) => {
  res.json({ ...(await getTodayStats()), whatsapp: getStatus().status, telegram: tgEnabled() });
});

app.get('/qr', async (req, res) => {
  const QRCode = require('qrcode');
  const { qrData } = getStatus();
